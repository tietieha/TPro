...
namespace UnityEditor.Polybrush
{
	...
	internal class BrushSettingsEditor : Editor
	{
		...
		{
            BrushSettings settings = (BrushSettings)target;
            ...
            if (showSettingsBounds)
            {
                using (new EditorGUI.IndentLevelScope())
                {
                    ...
                    {
                        brushRadiusMin.floatValue =
                            PolyGUILayout.FloatField(m_GCRadiusMin, brushRadiusMin.floatValue);
                        brushRadiusMin.floatValue =
                            Mathf.Clamp(brushRadiusMin.floatValue, .0001f, k_BrushSizeMaxValue);

                        brushRadiusMax.floatValue =
                            PolyGUILayout.FloatField(m_GCRadiusMax, brushRadiusMax.floatValue);
                        brushRadiusMax.floatValue = Mathf.Clamp(brushRadiusMax.floatValue,
                            brushRadiusMin.floatValue + .001f, k_BrushSizeMaxValue);

                        allowNonNormalizedFalloff.boolValue = PolyGUILayout.Toggle(m_GCAllowUnclampedFalloff,
                            allowNonNormalizedFalloff.boolValue);
                    }
                }
            }
            ...
            {
                radius.floatValue = PolyGUILayout.FloatFieldWithSlider(m_GCRadius, radius.floatValue,
                    brushRadiusMin.floatValue, brushRadiusMax.floatValue);
                strength.floatValue = PolyGUILayout.FloatFieldWithSlider(m_GCStrength, strength.floatValue, 0f, 1f);
            }
            if (EditorGUI.EndChangeCheck())
            {
                if (settings.isTexBrush)
                    settings.LoadBrush();
            }
            ...
            {
                // 贴图笔刷
                GUILayout.Label("笔刷图标", EditorStyles.boldLabel, new GUILayoutOption[0]);
                if (settings.PrepareBrushIcons())
                {
                    EditorGUI.BeginChangeCheck();
                    {
                        bool flag;
                        settings.currentSelectedTexBrush = PolyGUILayout.AspectSelectionGrid(
                            settings.currentSelectedTexBrush, settings.texBrushTextures, 0x20, "gridlist",
                    ...
                    settings.texBrushBlendMode =
                        (TEX_BLEND_MODE) EditorGUILayout.EnumPopup("混合模式", settings.texBrushBlendMode);
                    if (settings.texBrushBlendMode != TEX_BLEND_MODE.REPLACE)
                        settings.texBrushEraser = GUILayout.Toggle(settings.texBrushEraser, "橡皮擦",
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    if (allowNonNormalizedFalloff.boolValue)
                        curve.animationCurveValue =
                            EditorGUILayout.CurveField(curve.animationCurveValue, GUILayout.MinHeight(22));
                    else
                        curve.animationCurveValue = EditorGUILayout.CurveField(curve.animationCurveValue, Color.green,
                            RECT_ONE, GUILayout.MinHeight(22));
                }

                Keyframe[] keys = curve.animationCurveValue.keys;

                if ((approx(keys[0].time, 0f) && approx(keys[0].value, 0f) && approx(keys[1].time, 1f) &&
                     approx(keys[1].value, 1f)))
                {
                    Keyframe[] rev = new Keyframe[keys.Length];

                    for (int i = 0; i < keys.Length; i++)
                        rev[keys.Length - i - 1] = new Keyframe(1f - keys[i].time, keys[i].value, -keys[i].outTangent,
                            -keys[i].inTangent);
