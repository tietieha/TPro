...
namespace UnityEditor.Polybrush
{
    ...
    internal class PolybrushEditor : ConfigurableWindow
    {
        ...
        internal static PolybrushEditor instance
        {
        ...
        /// </summary>
        [UserSetting] internal static Pref<bool> s_FloatingWindow =
            new Pref<bool>("Editor.FloatingWindow", false, SettingsScope.Project);

        [SerializeField] List<BrushMode> modes = new List<BrushMode>();

        // A reference to the saved preset brush settings.
        [SerializeField] BrushSettings brushSettingsAsset;
        ...
        internal BrushTool tool = BrushTool.SplatTexture;
        ...
        BrushTool[] m_GCToolmodeTool = null;
        ...
        {
            get { return modes.Count > 0 ? modes[0] : null; }
            set
            {
        ...
        void OnEnable()
        {
            ...
            m_GCToolmodeIcons = new GUIContent[]
            {
                ...
                // EditorGUIUtility.TrIconContent(IconUtility.GetIcon("Toolbar/PaintVertexColors"),
                //     "Paint vertex colors on meshes"),
                // EditorGUIUtility.TrIconContent(IconUtility.GetIcon("Toolbar/PaintPrefabs"),
                //     "Scatter Prefabs on meshes"),
                // EditorGUIUtility.TrIconContent(IconUtility.GetIcon("Toolbar/PaintTextures"),
                //     "Paint textures on meshes"),
                EditorGUIUtility.TrIconContent(IconUtility.GetIcon("Toolbar/PaintSplatTextures"),
                    "Paint textures on Control Texture"),
            };

            m_GCToolmodeTool = new BrushTool[]
            {
                // BrushTool.RaiseLower,
                // BrushTool.Smooth,
                // BrushTool.Paint,
                // BrushTool.Prefab,
                // BrushTool.Texture,
                BrushTool.SplatTexture
            };
        ...
        void DrawToolbar()
        {
            ...
            {
                m_ToolbarIndex = GUILayout.Toolbar(m_ToolbarIndex, m_GCToolmodeIcons,
                    GUILayout.Width(s_Instance.position.width - 6), GUILayout.Height(23));
            }

            if (EditorGUI.EndChangeCheck())
            {
                BrushTool newTool = m_GCToolmodeTool[m_ToolbarIndex];
                SetTool(newTool == tool ? BrushTool.None : newTool);
                // SetTool(BrushTool.SplatTexture);
            }
        }
        ...
        void RefreshAvailableBrushList()
        {
            ...
            var brushSettingsName = brushSettings != null
                ? brushSettings.name
                : (EditorPrefs.HasKey(k_BrushSettingsName) ? EditorPrefs.GetString(k_BrushSettingsName) : String.Empty);
        ...
        internal void EnsureBrushSettingsListIsValid()
        {
            ...
            if (brushSettings == null)
            {
                if (brushSettingsAsset == null)
                    brushSettingsAsset =
                        BrushSettingsEditor.LoadBrushSettingsAssets(EditorPrefs.GetString(k_BrushSettingsAssetPref,
                            ""));
                ...
                {
                    SetBrushSettings(brushSettingsAsset != null
                        ? brushSettingsAsset
                        : PolyEditorUtility.GetFirstOrNew<BrushSettings>());
                }
            }

            brushSettings.isTexBrush = tool == BrushTool.SplatTexture;
            brushSettings.LoadBrush();
        }
        ...
        internal void SetBrushSettings(BrushSettings settings)
        {
            ...
            EditorPrefs.SetString(k_BrushSettingsAssetPref,
                AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(settings)));
            ...
            switch (e.GetTypeForControl(controlID))
            {
                case EventType.MouseMove:
                    ...
                    if (EditorApplication.timeSinceStartup - m_LastBrushUpdate > GetTargetFramerate(brushTarget))
                    {
                        ...
                        UpdateBrush(e.mousePosition, Event.current.control,
                            Event.current.shift && Event.current.type != EventType.ScrollWheel);
                    }
                ...
                case EventType.MouseDrag:
                    ...
                    if (EditorApplication.timeSinceStartup - m_LastBrushUpdate > GetTargetFramerate(brushTarget))
                    {
                        ...
                        UpdateBrush(e.mousePosition, Event.current.control,
                            Event.current.shift && Event.current.type != EventType.ScrollWheel);
                        ApplyBrush(Event.current.control,
                            Event.current.shift && Event.current.type != EventType.ScrollWheel);
                    }
                ...
                case EventType.KeyDown:
                    ...
                    if (((e.keyCode == KeyCode.LeftControl || e.keyCode == KeyCode.RightControl) &&
                         !brushSettings.isUserHoldingControl) ||
                        ((e.keyCode == KeyCode.LeftShift || e.keyCode == KeyCode.RightShift) &&
                         !brushSettings.isUserHoldingShift))
                    {
                        m_LastBrushUpdate = EditorApplication.timeSinceStartup;
                        UpdateBrush(e.mousePosition, Event.current.control,
                            Event.current.shift && Event.current.type != EventType.ScrollWheel);
                    ...
                    if (e.keyCode == KeyCode.LeftBracket)
                    {
                        ...
                        if (brushSettings != null)
                        {
                            brushSettings.radius -= m_movespeed;
                            m_movespeed += 0.1f;
                        }
                    }
                    ...
                    if (e.keyCode == KeyCode.Semicolon)
                    {
                        if (brushSettings != null)
                        {
                            brushSettings.strength -= m_movespeed;
                            m_movespeed += 0.01f;
                        }
                    }
                    if (e.keyCode == KeyCode.Quote)
                    {
                        if (brushSettings != null)
                        {
                            brushSettings.strength += m_movespeed;
                    ...
                    if (e.keyCode == KeyCode.Comma)
                    {
                        if (brushSettings != null)
                        {
                            brushSettings.falloff -= m_movespeed;
                            m_movespeed += 0.01f;
                        }
                    }
        ...
        bool TryEditingObject(GameObject go)
        {
            ...
            return EditorUtility.DisplayDialog("Mismatched application method",
                isObjectUsingAVS
                    ? "The object you are trying to edit doesn't support instanced mesh editing, do you want to convert it?"
                    : "The object you are trying to edit doesn't support additional vertex streams, do you want to convert it?",
                "Yes", "No");
        }
        ...
        internal void UpdateBrush(Vector2 mousePosition, bool isUserHoldingControl = false,
            bool isUserHoldingShift = false, bool isDrag = false, GameObject overridenGO = null,
            Ray? overridenRay = null)
        {
            ...
            if (tool == BrushTool.SplatTexture)
            {
                // 制作贴图笔刷，需要textureCoord，放到brushTarget里
                ...
                for (int i = 0; i < hits.Length; i++)
                {
                    ...
                    {
                        // ProjectorHelper.ProjectHelper.UpdateProjector(hitInfo.point, hitInfo.normal, Mathf.RoundToInt(brushSize));
                        // PreViewPaint(hitInfo.textureCoord, mixInfo);
                        brushTarget.textureCoord = hitInfo.textureCoord;
                        break;
                    }
                }
            }
                        ...
                        if (isValid)
                        {
                            ...
                            foreach (var hit in hits)
                            {
                                PolyRaycastHit secondaryHit = new PolyRaycastHit(hit.distance,
                                    secondaryTarget.transform.InverseTransformPoint(
                                        editable.transform.TransformPoint(hit.position)),
                                    hit.normal,
                                    -1);
        ...
        void ScrollBrushSettings(Event e)
        {
            ...
            switch (e.modifiers)
            {
                case EventModifiers.Control:
                    nrm = Mathf.Sin(Mathf.Max(.001f, brushSettings.normalizedRadius)) * .03f *
                          (brushSettings.brushRadiusMax - brushSettings.brushRadiusMin);
            ...
            if (mode != null)
            {
                if(brushSettings.isTexBrush)
                    brushSettings.LoadBrush();
                UpdateBrush(Event.current.mousePosition, Event.current.control,
                    Event.current.shift && Event.current.type != EventType.ScrollWheel);
                mode.OnBrushSettingsChanged(brushTarget, brushSettings);
            }
