...
namespace UnityEditor.Polybrush
{
    ...
    {
        static readonly string k_DefaultBrushColorGradient =
            "RGBA(0.227, 1.000, 0.227, 255.000)&0.000|RGBA(1.000, 1.000, 1.000, 255.000)&1.000|\n1.0000&0.000|0.2588&1.000|";
        ...
        [UserSetting("General Settings", "Inner Brush Color",
            "Vertices inside the brush's Inner Radius will be highlighted with this color")]
        internal static Pref<Color> s_FullStrengthColor =
            new Pref<Color>("Brush.BrushColor", k_DefaultBrushColor, SettingsScope.Project);

        [UserSetting("General Settings", "Brush Gradient",
            "Vertices outside the Inner Radius will be highlighted with this gradient, matching the brush's Falloff curve")]
        internal static Pref<Gradient> s_BrushGradientColor = new Pref<Gradient>("Brush.BrushColorGradient",
            GradientSerializer.Deserialize(k_DefaultBrushColorGradient), SettingsScope.Project);

        // The message that will accompany Undo commands for this brush.  Undo/Redo is handled by PolyEditor.
        internal virtual string UndoMessage
        {
            get { return "Apply Brush"; }
        }
        ...
        internal virtual void OnBrushEnter(EditableObject target, BrushSettings settings)
        {
            ...
            {
                // disable wirefame
                PolyEditorUtility.SetSelectionRenderState(target.renderer,
                    PolyEditorUtility.GetSelectionRenderState() & SelectionRenderState.Outline);
            }
        ...
        internal virtual void OnBrushExit(EditableObject target)
        {
            ...
            if (target.polybrushMesh != null && target.polybrushMesh.mode == PolybrushMesh.Mode.Mesh)
                DestroyImmediate(target.polybrushMesh);
        ...
        internal virtual void DrawGizmos(BrushTarget target, BrushSettings settings)
        {
            ...
            foreach (PolyRaycastHit hit in target.raycastHits)
                PolyHandles.DrawBrush(hit.position, hit.normal, settings, target.localToWorldMatrix, innerColor,
                    outerColor);
