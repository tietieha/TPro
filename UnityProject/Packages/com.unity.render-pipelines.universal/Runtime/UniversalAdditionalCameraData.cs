...
namespace UnityEngine.Rendering.Universal
{
	...
	public enum AntialiasingMode
	{
		...
		/// </summary>
		[InspectorName("No Anti-aliasing")] None,
	...
	public static class CameraExtensions
	{
		...
		internal static void GetVolumeLayerMaskAndTrigger(this Camera camera, UniversalAdditionalCameraData cameraData,
			out LayerMask layerMask, out Transform trigger)
		{
	...
	public class UniversalAdditionalCameraData : MonoBehaviour, ISerializationCallbackReceiver, IAdditionalData
	{
			...
			get
			{
				if (renderType != CameraRenderType.Base)
				{
					...
					Debug.LogWarning(string.Format(
						"{0}: This camera is of {1} type. Only Base cameras can have a camera stack.", camera.name,
						renderType));
					return null;
				}
		...
		internal void UpdateCameraStack()
		{
			...
			{
				Debug.LogWarning(name + ": " + removedCamsCount + " camera overlay" +
		...
		public enum CameraViewType
		{
			...
			[InspectorName("场景相机")] 
			SCENE      = 10, // 3D场景
			[InspectorName("场景UI相机")] 
			SCENE_UI   = 11, // 3D场景UI

			[InspectorName("RT渲染相机")] 
			RT         = 50, // 模型场景渲染到RT
		...
		[SerializeField] CameraViewType _cameraViewType = CameraViewType.UNKNOW;

		public CameraViewType cameraViewType
		{
			get => _cameraViewType;
			set => _cameraViewType = value;
		}
	}
}