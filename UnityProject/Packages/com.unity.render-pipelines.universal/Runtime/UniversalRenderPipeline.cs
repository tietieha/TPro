...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        static void RenderSingleCamera(ScriptableRenderContext context, ref CameraData cameraData, bool anyPostProcessingEnabled)
        {
            ...
            {
                renderer.Clear(cameraData.renderType, cameraData.cameraViewType);
        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera)
        {
            ...
            int needResolveToCameraTargetIndex = -1;
            if (cameraStack != null)
            {
                ...
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCamera.isActiveAndEnabled)
                    {
                        ...
                        if (data.cameraViewType > UniversalAdditionalCameraData.CameraViewType.MAINCAMERA && 
                            data.cameraViewType < UniversalAdditionalCameraData.CameraViewType.GUICAMERA)
                        {
	                        needResolveToCameraTargetIndex = i;
                        }
                    }
                }
            ...
            foreach ((Camera _, XRPass xrPass) in xrLayout.GetActivePasses())
            {
                ...
                // MainCamera的绘制
                // UW运行时只使用了MainCamera的Stack
                if (baseCameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.UNKNOW)
                ...
                if (isStackedRendering)
                {
                    for (int i = 0; i < cameraStack.Count; ++i)
                    {
                        ...
                        if (currAdditionalCameraData != null)
                        {
                            ...
                            overlayCameraData.resolveToCameraTarget = i == needResolveToCameraTargetIndex;
                            overlayCameraData.ClearFlagEX =
	                            overlayCameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.SCENE ||
	                            (overlayCameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.GUICAMERA && needResolveToCameraTargetIndex == -1)
		                            ? ClearFlag.All
		                            : ClearFlag.None; 
        ...
        static void InitializeAdditionalCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData, bool resolveFinalTarget, ref CameraData cameraData)
        {
            ...
            else if (additionalCameraData != null)
            {
                ...
                cameraData.cameraViewType = additionalCameraData.cameraViewType;
                // if (cameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.GUICAMERA)
                // {
	               //  cameraData.renderScale = 1.0f;
                //
	               //  cameraData.cameraTargetDescriptor.width = camera.pixelWidth;
	               //  cameraData.cameraTargetDescriptor.height = camera.pixelHeight;
                // }
            }
            else
            {
                ...
                cameraData.screenSizeOverride = cameraData.pixelRect.size;
                cameraData.screenCoordScaleBias = Vector2.one;
                cameraData.cameraViewType = UniversalAdditionalCameraData.CameraViewType.UNKNOW;
            }
