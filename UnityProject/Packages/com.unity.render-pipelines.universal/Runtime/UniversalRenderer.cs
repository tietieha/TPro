...
namespace UnityEngine.Rendering.Universal
{
    ...
    {
        #if !UNITY_EDITOR && (UNITY_SWITCH || UNITY_ANDROID)
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (cameraData.renderType == CameraRenderType.Base || 
                cameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.SCENE ||
                cameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.GUICAMERA)
            {
            ...
            else
            {
                ...
                ClearFlag opaqueForwardPassClearFlag = hasPassesBeforeOpaque ||
            ...
            if (camera.clearFlags == CameraClearFlags.Skybox && (cameraData.renderType != CameraRenderType.Overlay || cameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.SCENE))
            {
            ...
            if ((cameraData.renderType == CameraRenderType.Base || cameraData.cameraViewType == UniversalAdditionalCameraData.CameraViewType.SCENE) && !requiresDepthPrepass && !requiresDepthCopyPass)
            ...
            if (lastCameraInTheStack)
            {
                ...
                if (applyFinalPostProcessing && cameraData.cameraViewType != UniversalAdditionalCameraData.CameraViewType.GUICAMERA)
                {
            ...
            else
            {
	            if (applyPostProcessing)
	            {
		            postProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, false, m_ActiveCameraDepthAttachment, colorGradingLut,
			            m_MotionVectorColor, false, false);
		            EnqueuePass(postProcessPass);
	            }

	            // GUI Camera前的 绘制到屏幕上
	            if (cameraData.resolveToCameraTarget)
	            {
