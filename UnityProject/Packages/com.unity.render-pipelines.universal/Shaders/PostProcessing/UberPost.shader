Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local_fragment _ _HGAME_DISTORT
        ...
        TEXTURE2D(_HDistortMask);
        SAMPLER(sampler_HDistortMask);
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            #if _HGAME_DISTORT
                half3 HDistortTex = SAMPLE_TEXTURE2D_X(_HDistortMask, sampler_HDistortMask,uv).rgb;
                // uvDistorted = uvDistorted + pow(HDistortTex.xy,1/2.2f);
                uvDistorted = lerp(uvDistorted,uvDistorted + HDistortTex.xy * 2 - 1.0,HDistortTex.b);
                // uvDistorted = uvDistorted +( HDistortTex.xy * 2 - half2(1.0,1.0));
                // half3 xx = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uvDistorted).xyz;


                // half3 xx = SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uvDistorted).xyz;
                // xx = lerp(xx,half3( uvDistorted.xy,0),HDistortTex.b);

                // return half4(xx,1);
                // uvDistorted = lerp(uvDistorted,HDistortTex.rg,HDistortTex.b);
            #endif
            half3 color = (0.0).xxx;
