...
namespace UnityEditor
{
    ...
    public abstract class BaseShaderGUI : ShaderGUI
    {
        ...
        protected class Styles
        {
            ...
            public static readonly GUIContent blendControl = EditorGUIUtility.TrTextContent("Blend", $"Blend.");
            public static readonly GUIContent srcBlend = EditorGUIUtility.TrTextContent("SrcBlend", $"Src Blend.");
            public static readonly GUIContent dstBlend = EditorGUIUtility.TrTextContent("DstBlend", $"Dst Blend.");
            public static readonly string[] RenderBlendModeNames = Enum.GetNames(typeof(UnityEngine.Rendering.BlendMode));
        }

        #endregion
        
        public static readonly string BlendControl = "_BlendControl";
        ...
        /// <summary>
        /// The MaterialProperty for blendControl
        /// </summary>
        protected MaterialProperty blendProp { get; set; }
                
        /// <summary>
        /// The MaterialProperty for srcBlendProp
        /// </summary>
        protected MaterialProperty srcBlendProp { get; set; }
        
                
        /// <summary>
        /// The MaterialProperty for dstBlendProp
        /// </summary>
        ...
        public virtual void FindProperties(MaterialProperty[] properties)
        {
            ...
            blendProp = FindProperty(BlendControl, properties, false);
        ...
        public virtual void DrawSurfaceOptions(Material material)
        {
            ...
            else if ((surfaceTypeProp != null) && ((SurfaceType) surfaceTypeProp.floatValue == SurfaceType.Opaque))
            {
	            if(blendProp != null && srcBlendProp != null && dstBlendProp != null)
	            {
		            EditorGUI.indentLevel += 1;
		            DrawFloatToggleProperty(Styles.blendControl, blendProp);
		            var control = material.GetFloat(BlendControl) > 0f;
		            if (control)
		            {
			            EditorGUI.indentLevel += 1;
			            DoPopup(Styles.srcBlend, srcBlendProp, Styles.RenderBlendModeNames);
			            DoPopup(Styles.dstBlend, dstBlendProp, Styles.RenderBlendModeNames);
			            EditorGUI.indentLevel -= 1;
		            }
		            EditorGUI.indentLevel -= 1;
	            }
        ...
        internal static void SetupMaterialBlendModeInternal(Material material, out int automaticRenderQueue)
        {
            ...
            if (material.HasProperty(Property.SurfaceType))
            {
                ...
                if (surfaceType == SurfaceType.Opaque)
                {
                    ...
                    if (material.HasProperty(BlendControl) && material.GetFloat(BlendControl) > 0 && material.HasProperty(Property.SrcBlend) && material.HasProperty(Property.DstBlend))
                    {
	                    var src = (UnityEngine.Rendering.BlendMode) material.GetFloat(Property.SrcBlend);
	                    var dst = (UnityEngine.Rendering.BlendMode) material.GetFloat(Property.DstBlend);
	                    SetMaterialSrcDstBlendProperties(material, src, dst);
                    }
                    else
                    {
        ...
        private static string[] S_StripProperty = new[] {"_AnimInstanceOn", "_AnimMap", "_AnimTimeInfo", "_AnimInfo", "_PixelCountPerFrame" };
        private static Material S_SourceMaterial;
        private void CopyPropertiesGUI(Material material)
        {
	        EditorGUILayout.BeginHorizontal();
	        if (GUILayout.Button("复制"))
	        {
		        S_SourceMaterial = material;
	        }

	        var enable = GUI.enabled;
	        GUI.enabled = S_SourceMaterial != null && S_SourceMaterial.shader == material.shader;
	        if (GUILayout.Button("粘贴"))
	        {
		        int propertyCount = ShaderUtil.GetPropertyCount(S_SourceMaterial.shader);
		        for (int i = 0; i < propertyCount; i++)
		        {
			        string propertyName = ShaderUtil.GetPropertyName(S_SourceMaterial.shader, i);
			        if(S_StripProperty.Contains(propertyName)) continue;
			        ShaderUtil.ShaderPropertyType propertyType = ShaderUtil.GetPropertyType(material.shader, i);

			        switch (propertyType)
			        {
				        case ShaderUtil.ShaderPropertyType.Color:
					        material.SetColor(propertyName, S_SourceMaterial.GetColor(propertyName));
					        break;
				        case ShaderUtil.ShaderPropertyType.Vector:
					        material.SetVector(propertyName, S_SourceMaterial.GetVector(propertyName));
					        break;
				        case ShaderUtil.ShaderPropertyType.Float:
					        material.SetFloat(propertyName, S_SourceMaterial.GetFloat(propertyName));
					        break;
				        case ShaderUtil.ShaderPropertyType.Range:
                            material.SetFloat(propertyName, S_SourceMaterial.GetFloat(propertyName));
					        break;
				        case ShaderUtil.ShaderPropertyType.TexEnv:
                            material.SetTexture(propertyName, S_SourceMaterial.GetTexture(propertyName));
					        break;
				        case ShaderUtil.ShaderPropertyType.Int:
                            material.SetInt(propertyName, S_SourceMaterial.GetInt(propertyName));
					        break;
			        }
		        }
	        }
