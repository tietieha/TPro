...
public class CommandLineReader
{
    ...
    static Dictionary<string, string> defaultArgsDict = new Dictionary<string, string>();
    ...
    public static string GetCustomArgument(string argumentName)
    {
        ...
        {
            if (defaultArgsDict.TryGetValue(argumentName, out var defaultValue))
            {
                Debug.Log($"[CommandLineReader] - GetCustomArgument - Custom argument [{argumentName}] not found, returning default value [{defaultValue}].");
                return defaultValue;
            }
    ...
    public static void SetDefaultCustomArgument(string argumentName, string defaultValue)
    {
        if (!defaultArgsDict.ContainsKey(argumentName))
        {
            defaultArgsDict.Add(argumentName, defaultValue);
        }
        else
        {
            Debug.LogWarning($"[CommandLineReader] - SetDefaultCustomArgument - Custom argument [{argumentName}] already exists with value [{defaultArgsDict[argumentName]}]. Overwriting with new value [{defaultValue}].");
            defaultArgsDict[argumentName] = defaultValue;
        }
    }
}
