...
namespace TEngine.Editor.Inspector
{
    ...
    internal sealed class ResourceModuleInspector : GameFrameworkInspector
    {
        ...
        private SerializedProperty m_MaxCarrierDataNetworkSizeMb = null;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlaying);
            {
                ...
                float maxCarrierDataNetworkSizeMb = EditorGUILayout.DelayedFloatField("Max CarrierDataNetwork SizeMb", m_MaxCarrierDataNetworkSizeMb.floatValue);
                if (Math.Abs(maxCarrierDataNetworkSizeMb - m_MaxCarrierDataNetworkSizeMb.floatValue) > 0.01f)
                {
                    if (EditorApplication.isPlaying)
                    {
                        t.MaxCarrierDataNetworkSizeMb = maxCarrierDataNetworkSizeMb;
                    }
                    else
                    {
                        m_MaxCarrierDataNetworkSizeMb.floatValue = maxCarrierDataNetworkSizeMb;
                    }
                }
            }
            EditorGUI.EndDisabledGroup();
        ...
        private void OnEnable()
        {
            ...
            m_MaxCarrierDataNetworkSizeMb = serializedObject.FindProperty("m_MaxCarrierDataNetworkSizeMb");
