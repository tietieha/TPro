...
namespace TEngine
{
    public partial class ResourceExtComponent
    {
        ...
        private void OnLoadAssetFailure(string assetName, LoadResourceStatus status, string errormessage, int guid, object userdata)
        {
            _assetLoadingList.Remove(assetName);
            Log.Error("Can not load asset from '{1}' with error message '{2}'.", assetName, errormessage);
        }

        private void OnLoadAssetSuccess(string assetName, object asset, int guid, object userdata)
        {
            _assetLoadingList.Remove(assetName);
