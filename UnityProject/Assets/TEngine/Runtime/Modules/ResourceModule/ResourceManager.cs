...
namespace TEngine
{
    ...
    internal sealed partial class ResourceManager : ModuleImp, IResourceManager
    {
        ...
        public async UniTask<InitializationOperation> InitPackage(string packageName)
        {
            ...
            Log.Debug($"Editor Module Used :{playMode}");
#else
            //运行时使用。
            ...
            if (playMode == EPlayMode.HostPlayMode)
            {
                ...
                FallbackHostServerURL = HostServerURL;
                Log.Debug($"HostPlayMode HostServerURL : {HostServerURL}, FallbackHostServerURL : {FallbackHostServerURL}");
            ...
            if (playMode == EPlayMode.WebPlayMode)
            {
                HostServerURL = SettingsUtils.GetResDownLoadPath();
                FallbackHostServerURL = HostServerURL;
                Log.Debug($"WebPlayMode HostServerURL : {HostServerURL}, FallbackHostServerURL : {FallbackHostServerURL}");
                string defaultHostServer = HostServerURL;
                string fallbackHostServer = FallbackHostServerURL;
        ...
        public Object LoadAsset(string location, Type assetType, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
            ...
            if (assetObject != null)
            {
                return assetObject.Target as Object;
            }

            AssetHandle handle = GetHandleSync(location, assetType, packageName: packageName);

            Object ret = handle.AssetObject as Object;
        ...
        /// <param name="packageName">指定资源包的名称。不传使用默认资源包。</param>
        public async void LoadAssetAsync(string location, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, int guid, object userData, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
            ...
            {
                await UniTask.Yield();
                loadAssetCallbacks.LoadAssetSuccessCallback(location, assetObject.Target, guid, userData);
                return;
            }
            ...
            {
                string errorMessage = Utility.Text.Format("Can not load asset guid:'{0}' location:'{1}' because :'{2}'.", guid, location, assetInfo.Error);
                if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                {
                    loadAssetCallbacks.LoadAssetFailureCallback(location, LoadResourceStatus.NotExist, errorMessage, guid, userData);
                    return;
                }
            ...
            if (handle.AssetObject == null || handle.Status == EOperationStatus.Failed)
            {
                ...
                if (loadAssetCallbacks.LoadAssetFailureCallback != null)
                {
                    loadAssetCallbacks.LoadAssetFailureCallback(location, LoadResourceStatus.NotReady, errorMessage, guid, userData);
                    return;
                }
            ...
            else
            {
                ...
                if (loadAssetCallbacks.LoadAssetSuccessCallback != null)
                {
                    ...
                    loadAssetCallbacks.LoadAssetSuccessCallback(location, handle.AssetObject, guid, userData);
                }
            }
        }
        ...
        /// <param name="userData">用户自定义数据。</param>
        /// <param name="packageName">指定资源包的名称。不传使用默认资源包。</param>
        public async void LoadAssetAsync(string location, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, CancellationToken cancellationToken, int guid, object userData = null, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
        ...
        /// <param name="userData">用户自定义数据。</param>
        /// <param name="packageName">指定资源包的名称。不传使用默认资源包。</param>
        public async void LoadAssetAsync(string location, int priority, LoadAssetCallbacks loadAssetCallbacks, int guid, object userData, string packageName = "")
        {
            if (string.IsNullOrEmpty(location))
            {
                throw new GameFrameworkException("Asset name is invalid.");
