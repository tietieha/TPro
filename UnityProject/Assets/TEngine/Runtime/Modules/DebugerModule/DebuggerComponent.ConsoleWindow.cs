...
namespace TEngine
{
    public sealed partial class DebuggerModule : Module
    {
        ...
        private sealed class ConsoleWindow : IDebuggerWindow
        {
            ...
            private float _stackRectHeight = 100f;
            private int _infoCount = 0;
            private int _warningCount = 0;
            private int _errorCount = 0;
            private int _fatalCount = 0;
            private LogNode _selectedNode = null;
            private bool _lastLockScroll = false;
            private bool _lastLogStack = false;
            private bool _lastInfoFilter = false;
            private bool _lastWarningFilter = false;
            private bool _lastErrorFilter = true;
            private bool _lastFatalFilter = true;

            private float oldDrag = 0;
            private float oldDragStack = 0;

            [SerializeField] private bool m_LockScroll = false;

            [SerializeField] private bool m_LogStack = false;

            [SerializeField] private int m_MaxLine = 100;

            [SerializeField] private bool m_InfoFilter = false;

            [SerializeField] private bool m_WarningFilter = false;
            ...
            public void Initialize(params object[] args)
            {
                ...
                m_LogStack = _lastLogStack = _settingModule.GetBool("Debugger.Console.LogStack", false);
                m_InfoFilter = _lastInfoFilter = _settingModule.GetBool("Debugger.Console.InfoFilter", false);
                m_WarningFilter = _lastWarningFilter = _settingModule.GetBool("Debugger.Console.WarningFilter", false);
            ...
            public void OnUpdate(float elapseSeconds, float realElapseSeconds)
            {
                ...
                if (_lastLogStack != m_LogStack)
                {
                    _lastLogStack = m_LogStack;
                    _settingModule.SetBool("Debugger.Console.LogStack", m_LogStack);
                }
            ...
            {
                var stackRectHeight = contentRect.height * (m_LogStack ? 0.33f : 0.1f);
                var toolHeight = contentRect.height * 0.03f * windowScale;
                var toolRect = new Rect(contentRect.x, contentRect.y, contentRect.width, toolHeight);
                var logsRect = new Rect(contentRect.x, toolRect.y + toolHeight, contentRect.width,
                    contentRect.height - stackRectHeight - toolHeight);
                var stackRect = new Rect(contentRect.x, logsRect.y + logsRect.height, contentRect.width,
                    stackRectHeight);
                RefreshCount();
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    m_LogStack = GUILayout.Toggle(m_LogStack, "Log Stack", GUILayout.Width(90f));
                    GUILayout.FlexibleSpace();
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, Utility.Text.Format("Info ({0})", _infoCount),
                        GUILayout.Width(90f));
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter,
                        Utility.Text.Format("Warning ({0})", _warningCount), GUILayout.Width(90f));
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, Utility.Text.Format("Error ({0})", _errorCount),
                        GUILayout.Width(90f));
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, Utility.Text.Format("Fatal ({0})", _fatalCount),
                        GUILayout.Width(90f));
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    var isDrag = false;
                    if (touch.y != 0 && logsRect.Contains(new Vector2(touch.z, touch.w)))
                    {
                        _logScrollPosition.y += (touch.y - oldDrag);
                        isDrag = (touch.y - oldDrag) > 0.5;
                    }
                ...
                GUILayout.BeginArea(stackRect);
                GUILayout.BeginVertical("box");
                {
                    if (touch.y != 0 && stackRect.Contains(new Vector2(touch.z, touch.w)))
                    {
                        _stackScrollPosition.y += (touch.y - oldDragStack);
                    }

                    _stackScrollPosition =
                        GUILayout.BeginScrollView(_stackScrollPosition, GUILayout.Height(stackRectHeight));
                    {
                        oldDragStack = touch.y;
