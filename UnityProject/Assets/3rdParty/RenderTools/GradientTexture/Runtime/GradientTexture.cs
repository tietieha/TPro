...
namespace Packages.GradientTextureGenerator.Runtime
{
	...
	{
		[SerializeField] Vector2Int _resolution = new Vector2Int(256, 32);
		[SerializeField] bool _sRGB = true;
		[SerializeField] AnimationCurve _verticalLerp = AnimationCurve.Linear(0, 0, 1, 1);
		[SerializeField, GradientUsage(false)] Gradient _horizontalTop = GetDefaultGradient();
		[SerializeField, GradientUsage(false)] Gradient _horizontalBottom = GetDefaultGradient();
...
#if UNITY_EDITOR
			if (!_texture) return;
			var serializedObject = new SerializedObject(_texture);
			var serializedProperty = serializedObject.FindProperty("m_IsReadable");
			serializedProperty.boolValue = isReadable;
