	...
	public partial class PrefabPainter : EditorWindow
	{
		...
		public class Octree
		{
			...
			class Node
			{
				...
				const float
					kNodesOverlapSize = 0.001f; // fixes situations where ray lies between nodes
				...
				public void Resize(Bounds treeBounds)
				{
					...
					Vector3 childSize = treeBounds.extents + new Vector3(kNodesOverlapSize,
						kNodesOverlapSize, kNodesOverlapSize);
					;

					childs[0].Resize(new Bounds(
						new Vector3(center.x + offset.x, center.y + offset.y, center.z + offset.z),
						childSize));
					childs[1].Resize(new Bounds(
						new Vector3(center.x + offset.x, center.y + offset.y, center.z - offset.z),
						childSize));
					childs[2].Resize(new Bounds(
						new Vector3(center.x + offset.x, center.y - offset.y, center.z + offset.z),
						childSize));
					childs[3].Resize(new Bounds(
						new Vector3(center.x + offset.x, center.y - offset.y, center.z - offset.z),
						childSize));
					childs[4].Resize(new Bounds(
						new Vector3(center.x - offset.x, center.y + offset.y, center.z + offset.z),
						childSize));
					childs[5].Resize(new Bounds(
						new Vector3(center.x - offset.x, center.y + offset.y, center.z - offset.z),
						childSize));
					childs[6].Resize(new Bounds(
						new Vector3(center.x - offset.x, center.y - offset.y, center.z + offset.z),
						childSize));
					childs[7].Resize(new Bounds(
						new Vector3(center.x - offset.x, center.y - offset.y, center.z - offset.z),
						childSize));
				}
			...
			delegate bool HandleUtility_IntersectRayMesh(Ray ray, Mesh mesh, Matrix4x4 matrix,
			...
			static Octree()
			{
				MethodInfo methodIntersectRayMesh =
					typeof(HandleUtility).GetMethod("IntersectRayMesh",
						BindingFlags.Static | BindingFlags.NonPublic);

				if (methodIntersectRayMesh != null)
				{
					IntersectRayMesh = delegate(Ray ray, Mesh mesh, Matrix4x4 matrix,
						out UnityEngine.RaycastHit raycastHit)
					{
			...
			public void AddObject(GameObject gameobject, bool useAdditionalVertexStreams = false)
			{
				OctreeObject octreeObject =
					MakeOctreeObject(gameobject, useAdditionalVertexStreams);
				if (octreeObject != null)
					m_Tree.AddObject(octreeObject);
			}
			...
			OctreeObject MakeOctreeObject(GameObject gameObject, bool useAdditionalVertexStreams)
			{
				         ...
				         meshFilter != null && meshFilter.sharedMesh != null)
				{
					...
					if (useAdditionalVertexStreams && meshRenderer != null &&
				...
				else if ((collider = gameObject.GetComponent<Collider>()) != null &&
				         collider.enabled)
				{
					OctreeObject obj = new OctreeObject();
			...
			public void Populate(GameObject[] sceneObjects, bool useAdditionalVertexStreams)
			{
				...
				var mapW = MapRender.instance.map.mapSizeW;
				var mapH = MapRender.instance.map.mapSizeH;
				Bounds worldBounds = new Bounds(new Vector3(mapW / 2f, -10, -mapH / 2f),
					new Vector3(mapW, 100, mapH));
				...
				for (int i = 0; i < sceneObjects.Length; i++)
				{
					...
					OctreeObject octreeObject =
						MakeOctreeObject(sceneObjects[i], useAdditionalVertexStreams);
					if (octreeObject != null)
					{
			...
			static bool RaycastTriangle(Ray ray, Vector3 p1, Vector3 p2, Vector3 p3, out float u,
			...
			public static bool IntersectRayMeshEx(Ray ray, Mesh mesh, Matrix4x4 matrix,
				out RaycastInfo raycastInfo)
			{
				    ...
				    IntersectRayMesh(ray, mesh, matrix, out unityRaycastHit))
				{
					...
					raycastInfo.localNormal = matrix.inverse.MultiplyVector(unityRaycastHit.normal)
						.normalized;
					raycastInfo.localPoint = matrix.inverse.MultiplyPoint(unityRaycastHit.point);

					raycastInfo.point = unityRaycastHit.point;
					raycastInfo.normal = matrix.inverse.transpose
						.MultiplyVector(raycastInfo.localNormal).normalized;
			...
			void Raycast(ref RaycastData raycastData, Node node)
			{
				...
				for (int i = 0; i < node.objects.Count; i++)
				{
					...
					bool isObjectInList = raycastData.objectList != null &&
			...
			void SortedRaycast(ref RaycastData raycastData)
			{
				for (int i = 0; i < m_SortedObjects.Count; i++)
				{
					...
					if (obj.mesh != null)
					{
						if (IntersectRayMeshEx(raycastData.ray, obj.mesh,
							    obj.gameObject.transform.localToWorldMatrix, out raycastInfo))
						{
					...
					else if (obj.rectTransform != null)
					{
						...
						if (RaycastTriangle(raycastData.ray, obj.cornerPoints[0],
							    obj.cornerPoints[1], obj.cornerPoints[2], out u, out v, out t) ||
						    RaycastTriangle(raycastData.ray, obj.cornerPoints[0],
							    obj.cornerPoints[2], obj.cornerPoints[3], out u, out v, out t))
						{
							if (t < raycastData.raycastInfo.distance)
							{
								...
								raycastInfo.normal = (new Plane(obj.cornerPoints[0],
									obj.cornerPoints[1], obj.cornerPoints[2])).normal;
			...
			public bool Raycast(Ray ray, out RaycastInfo raycastInfo, int layersMask,
				int ignoreLayersMask, List<GameObject> objectList)
			{
				s_RaycastOpID++;
				raycastCounter++;
		...
		static int s_BrushToolHash = "MapEditor.PrefabPainter.BrushTool".GetHashCode();
		static int s_PinToolHash = "MapEditor.PrefabPainter.PinTool".GetHashCode();
		static int s_ThrowToolHash = "MapEditor.PrefabPainter.ThrowTool".GetHashCode();
		static int s_PlaceToolHash = "MapEditor.PrefabPainter.PlaceTool".GetHashCode();
		static int s_EraseToolHash = "MapEditor.PrefabPainter.EraseTool".GetHashCode();
		static int s_SelectToolHash = "MapEditor.PrefabPainter.SelectTool".GetHashCode();
		static int s_ModifyToolHash = "MapEditor.PrefabPainter.ModifyTool".GetHashCode();
		static int s_OrientToolHash = "MapEditor.PrefabPainter.OrientTool".GetHashCode();
		static int s_MoveToolHash = "MapEditor.PrefabPainter.SlideTool".GetHashCode();
		static int s_PickObjectToolHash = "MapEditor.PrefabPainter.PickObjectTool".GetHashCode();
		...
		PrefabPainterSettings LoadSettings()
		{
			...
			string settingsFileName = $"{map.Name}_{kSettingsFileName}";
			// Try load settings asset
			PrefabPainterSettings settings = AssetDatabase.LoadAssetAtPath(
		...
		void EditorApplicationUpdateCallback()
		{
			...
			{
				if (Time.realtimeSinceStartup - m_LastPhysicsUpdateTime >
		...
		bool Raycast(Ray ray, out RaycastInfo raycastInfo, int layersMask, int ignoreLayersMask)
		{
			...
			if (m_Settings.paintOnSelected && (m_CurrentTool == PaintTool.Brush ||
			                                   m_CurrentTool == PaintTool.Pin ||
			                                   m_CurrentTool == PaintTool.Place))
			{
		...
		Quaternion OrientObject(OrientationMode orientationMode, Vector3 normal,
		...
		void OrientObject(PlacedObjectInfo placedObjectInfo, Vector3 euler)
		{
			...
			Quaternion placeOrientation =
				OrientObject(brushSettings.orientationMode, normal, isRectTransform);
		...
		void BrushModeOrientObject(PlacedObjectInfo placedObjectInfo)
		{
			...
			randomRotation = Quaternion.Euler(new Vector3(
		...
		void PositionObject(PlacedObjectInfo info)
		{
			...
			Vector3 pivot = GetObjectPivot(info.pivotObject,
			...
			if (brushSettings.multibrushEnabled)
			{
				...
				transform.position = transform.position + transform.right * offset.x +
		...
		void ScaleObject(PlacedObjectInfo info)
		{
			...
			randomVector = new Vector3(Mathf.Abs(randomVector.x), Mathf.Abs(randomVector.y),
				Mathf.Abs(randomVector.z));

			if (brushSettings.scaleMode == AxisMode.Uniform)
			{
				float scaleValue = brushSettings.scaleUniformMin + randomVector.x *
			...
			{
				scale = new Vector3(
					brushSettings.scalePerAxisMin.x + randomVector.x *
					(brushSettings.scalePerAxisMax.x - brushSettings.scalePerAxisMin.x),
					brushSettings.scalePerAxisMin.y + randomVector.y *
					(brushSettings.scalePerAxisMax.y - brushSettings.scalePerAxisMin.y),
					brushSettings.scalePerAxisMin.z + randomVector.z *
		...
		PlacedObjectInfo PlaceObject(RaycastInfo raycastInfo, Brush brush)
		{
			...
			GameObject gameObject =
				PrefabUtility.InstantiatePrefab(brush.prefabSlots[prefabSlot].gameObject) as
					GameObject;
			if (gameObject == null)
				return null;
			...
			{
				Utility.ForAllInHierarchy(pivotObject,
					go => { go.layer = m_Settings.prefabPlaceLayer; });
			}
		...
		PlacedObjectInfo BrushModePlaceObject(RaycastInfo raycastInfo, Brush brush)
		{
			...
			if (brush.settings.brushOverlapCheckMode != OverlapCheckMode.None)
			{
				...
				if (GetObjectWorldBounds(placedObjectInfo.pivotObject, out bounds) &&
			...
			AddObject(placedObjectInfo.gameObject, m_Settings.useAdditionalVertexStreams);
		...
		{
			GameObject placeLocation = GetPlaceObjectParent();
		...
		GameObject GetPlaceObjectParent()
		{
			GameObject placeLocation = MapRender.instance.parentForPrefabs;
			...
			// 		if (StageUtility.GetCurrentStage() == StageUtility.GetMainStage())
			// 			placeLocation = MapRender.instance.parentForPrefabs;
			// 		break;
			// 	default:
			...
			if (m_Settings.groupPrefabs)
			{
				...
				string groupName = m_Settings.GetActiveTab().name + m_Settings.groupName;
		...
		void GetOrientation(Vector3 surfaceNormal, OrientationMode mode, out Vector3 upwards,
		...
		public static float PointLineDistance(Vector3 point, Vector3 linePoint,
			Vector3 lineDirection)
		{
			float q = (Vector3.Dot(lineDirection, (point - linePoint))) /
		...
		float GetObjectScaleFactor(GameObject gameObject)
		{
			...
			size = Mathf.Max(PointLineDistance(new Vector3(min.x, min.y, min.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(min.x, min.y, max.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(min.x, max.y, min.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(min.x, max.y, max.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(max.x, min.y, min.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(max.x, min.y, max.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(max.x, max.y, min.z), point, direction),
				size);
			size = Mathf.Max(PointLineDistance(new Vector3(max.x, max.y, max.z), point, direction),
				size);
		...
		void DrawBrushHandles(RaycastInfo hit, Brush brush)
		{
			...
			if (brush.settings.gridEnabled)
			{
				...
				Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
					Mathf.Min(brush.settings.gridStep.x, brush.settings.gridStep.y) * 0.5f,
					EventType.Repaint);

				Handles.color = new Color(0.5f, 0.5f, 0.5f, 0.1f);
				Handles.DrawSolidDisc(hit.point, hit.normal,
					Mathf.Min(brush.settings.gridStep.x, brush.settings.gridStep.y) * 0.5f);
			}
			else
			{
				Handles.color = m_Settings.handlesColor;
				Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
					brush.settings.brushRadius, EventType.Repaint);
			...
			GetOrientation(hit.normal, brush.settings.orientationMode, out upwards, out right,
				out forward);
		...
		void DrawEraseHandles(RaycastInfo hit, Brush brush)
		{
			...
			Handles.color = m_Settings.handlesColor;
			Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
				m_Settings.eraseBrushRadius, EventType.Repaint);
		...
		void DrawSelectHandles(RaycastInfo hit, Brush brush)
		{
			...
			Handles.color = m_Settings.handlesColor;
			Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
				m_Settings.selectBrushRadius, EventType.Repaint);
		...
		void DrawModifyHandles(RaycastInfo hit, Brush brush)
		{
			...
			Handles.color = m_Settings.handlesColor;
			Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
				m_Settings.modifyBrushRadius, EventType.Repaint);
		...
		void DrawMoveToolHandles(bool isHot)
		{
			...
			if (m_MoveTool.objects.Count > 0)
			{
				...
				Vector3 normal = (m_MoveTool.objectsCenter - Camera.current.transform.position)
					.normalized;

				Handles.color = new Color(m_Settings.handlesColor.r, m_Settings.handlesColor.g,
					m_Settings.handlesColor.b, 0.2f);
		...
		void DrawMaskedHandles(RaycastInfo hit, Brush brush)
		{
			...
			Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
				handleSize * 0.5f, EventType.Repaint);
			Handles.DrawLine(hit.point + right * handleSize, hit.point + right * -handleSize);
			Handles.DrawLine(hit.point + forward * handleSize, hit.point + forward * -handleSize);
		...
		void DrawPickObjectHandles(RaycastInfo hit)
		{
			...
			Handles.ArrowHandleCap(0, hit.point, Quaternion.LookRotation(hit.normal, right),
				handleSize * 2f, EventType.Repaint);
			Handles.color = Color.red;
			Handles.DrawAAPolyLine(3, hit.point + right * handleSize,
				hit.point + right * -handleSize);
			Handles.color = Color.blue;
			Handles.DrawAAPolyLine(3, hit.point + forward * handleSize,
		...
		void DrawPinToolHandles(Brush brush)
		{
			...
			Handles.color = m_Settings.handlesColor;
			Handles.CircleHandleCap(1, m_PinTool.placedObjectInfo.raycastInfo.point,
				Quaternion.LookRotation(upwards), m_PinTool.radius, EventType.Repaint);
			Handles.DrawDottedLine(m_PinTool.placedObjectInfo.raycastInfo.point, m_PinTool.point,
				4.0f);
			Handles.DrawDottedLine(m_PinTool.placedObjectInfo.raycastInfo.point,
				m_PinTool.placedObjectInfo.raycastInfo.point + forward * m_PinTool.radius, 4.0f);

			Handles.color = new Color(0, 0, 0, 0.5f);
			Handles.DrawSolidArc(m_PinTool.placedObjectInfo.raycastInfo.point, upwards, forward,
				m_PinTool.angle, m_PinTool.radius);

			Handles.color = m_Settings.handlesColor;
			Styles.handlesBoldTextStyle.normal.textColor = m_Settings.handlesColor;
			Handles.Label(m_PinTool.point, "    Angle: " + (m_PinTool.angle).ToString("F2"),
		...
		void DrawPlaceToolHandles(Brush brush)
		{
			...
			Handles.DrawWireCube(m_PlaceTool.placedObjectInfo.localBounds.center,
				m_PlaceTool.placedObjectInfo.localBounds.size);
			Handles.matrix = Matrix4x4.identity;

			Handles.color = m_Settings.handlesColor;
			Styles.handlesBoldTextStyle.normal.textColor = m_Settings.handlesColor;
			Handles.Label(m_PlaceTool.raycastInfo.point, "    Rotation: X: " +
			                                             (brush.settings.placeEulerAngles.x)
			                                             .ToString("F2")
			                                             + " Y: " +
			                                             (brush.settings.placeEulerAngles.y)
			                                             .ToString("F1")
			                                             + " Z: " +
			                                             (brush.settings.placeEulerAngles.z)
			                                             .ToString("F1"),
				Styles.handlesBoldTextStyle);
		...
		void DrawDropHandles(RaycastInfo hit, Brush brush)
		{
			...
			Handles.color = m_Settings.handlesColor;
			Handles.CircleHandleCap(1, hit.point, Quaternion.LookRotation(hit.normal),
				brush.settings.throwRadius, EventType.Repaint);
		...
		void DrawXYZCross(RaycastInfo hit, Vector3 upwards, Vector3 right, Vector3 forward)
		{
			...
			Handles.color = Color.red;
			Handles.DrawAAPolyLine(lineWidth, hit.point + right * handleSize,
				hit.point + right * -handleSize * 0.2f);
			Handles.color = Color.green;
			Handles.DrawAAPolyLine(lineWidth, hit.point + upwards * handleSize,
				hit.point + upwards * -handleSize * 0.2f);
			Handles.color = Color.blue;
			Handles.DrawAAPolyLine(lineWidth, hit.point + forward * handleSize,
		...
		void DrawGrid(Brush brush)
		{
			...
			Matrix4x4 gridMatrix = Matrix4x4.TRS(m_Grid.visualOrigin,
				Quaternion.AngleAxis(brush.settings.gridAngle, gridNormal) *
				Quaternion.LookRotation(gridNormal), gridStep);
			...
			for (int x = -halfSize; x <= halfSize; x++)
			{
				for (int y = -halfSize; y <= halfSize; y++)
				{
					...
					float alpha =
						gridBaseAlpha *
						Mathf.Clamp01((1.0f - (localPoint - localHitPoint).magnitude *
							halfSizeInv)) * 1.0f + 0.15f;
					Handles.color = new Color(1, 1, 1, alpha) * m_Settings.handlesColor;
					Handles.DotHandleCap(0, point, Quaternion.identity,
						HandleUtility.GetHandleSize(point) * 0.03f, EventType.Repaint);
				}
			}
			...
			if (!m_Grid.inDeadZone)
			{
				// Draw origin point
				Handles.color = m_Settings.handlesColor;
				Handles.DotHandleCap(0, m_Grid.originRaycastInfo.point, Quaternion.identity,
		...
		{
			Vector3 gridOrigin =
				new Vector3(brush.settings.gridOrigin.x, brush.settings.gridOrigin.y, 0);
			Vector3 gridStep = new Vector3(brush.settings.gridStep.x, brush.settings.gridStep.y, 1);
			Vector3 gridNormal = GetGridNormalVector(brush);
			...
			Matrix4x4 gridMatrix = Matrix4x4.TRS(Vector3.zero,
				                       ...
				                       Quaternion.LookRotation(gridNormal), Vector3.one)
			...
			m_Grid.inDeadZone =
				(new Vector2(Mathf.Round(gridSpacePoint.x), Mathf.Round(gridSpacePoint.y)) -
				 new Vector2(gridSpacePoint.x, gridSpacePoint.y)).magnitude > Grid.kDeadZoneSize;


			// round point values
			gridSpacePoint = new Vector3(Mathf.Round(gridSpacePoint.x),
				Mathf.Round(gridSpacePoint.y), gridSpacePoint.z);
			...
			Raycast(ray, out m_Grid.originRaycastInfo, m_Settings.paintLayers.value,
			...
			if (!m_Grid.originRaycastInfo.isHit)
			{
				for (int i = 0; i < 3; i++)
				{
					Ray shiftedRay = new Ray(ray.origin + UnityEngine.Random.onUnitSphere * 0.001f,
						ray.direction);
					Raycast(shiftedRay, out m_Grid.originRaycastInfo, m_Settings.paintLayers.value,
						m_Settings.ignoreLayers.value);
					if (m_Grid.originRaycastInfo.isHit)
						break;
				}
			...
			Raycast(m_Grid.originRaycastInfo.ray, out visualRaycast, m_Settings.paintLayers.value,
		...
		void OnToolEnabled(PaintTool tool)
		{
			...
			if (m_CurrentStage == StageUtility.GetMainStage())
			{
                ...
                sceneObjects =
 GameObject.FindObjectsByType<GameObject>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
#else
		...
		void OnToolDisabled(PaintTool tool)
		{
			...
			switch (tool)
			{
				...
				case PaintTool.Throw:
				{
					foreach (var rigidbody in m_ThrowTool.m_rigidbodys.Where(rigidbody =>
						         rigidbody != null))
					{
						rigidbody.isKinematic = false;
					}

					foreach (var rigidbody2D in m_ThrowTool.m_rigidbody2Ds.Where(rigidbody2D =>
						         rigidbody2D != null))
					{
		...
		void DoPickObject()
		{
			...
			switch (e.GetTypeForControl(controlID))
			{
				case EventType.MouseDown:
					if (e.button == 0)
					{
						Raycast(HandleUtility.GUIPointToWorldRay(e.mousePosition),
		...
		void DoBrushTool()
		{
			bool hasMultipleSelectedBrushes =
				m_Settings.GetActiveTab().HasMultipleSelectedBrushes();
			...
			switch (e.GetTypeForControl(controlID))
			{
					...
					if (e.button == 0 && !e.alt && brush != null && !hasMultipleSelectedBrushes)
					{
						...
						if (brush.settings.gridEnabled)
						{
							PlacedObjectInfo placedObjectInfo =
								BrushModePlaceObject(m_BrushTool.raycastInfo, brush);
							if (placedObjectInfo != null)
							{
						...
						else if (m_BrushTool.raycastInfo.isHit)
						{
							...
							Vector3 randomPoint = m_CurrentRaycast.point +
							                      Vector3.ProjectOnPlane(
								                      ...
								                      m_CurrentRaycast.normal) * brushRadius;

							Raycast(WorldPointToRay(randomPoint), out raycastInfo,
								m_Settings.paintLayers.value, m_Settings.ignoreLayers.value);
				...
				case EventType.MouseDrag:
					if (GUIUtility.hotControl == controlID && e.button == 0 && brush != null &&
					    !hasMultipleSelectedBrushes)
					{
						...
						if (m_BrushTool.firstNormal.magnitude < 0.1f &&
						...
						if (m_BrushTool.raycastInfo.isHit || m_BrushTool.prevRaycast.isHit)
						{
							...
							{
								if (!m_BrushTool.prevRaycast.IntersectsHitPlane(
									    m_BrushTool.raycastInfo.ray, out hitPoint))
									isTwoPoints = false;
							}
							...
							{
								if (!m_BrushTool.raycastInfo.IntersectsHitPlane(
									    m_BrushTool.prevRaycast.ray, out lastHitPoint))
									isTwoPoints = false;
							}
							...
							if (brush.settings.gridEnabled)
							{
								if (m_BrushTool.raycastInfo.isHitTargetLayer &&
								    !m_Grid.inDeadZone &&
								    (m_BrushTool.lastPlacedObjectInfo == null ||
								     !Utility.IsVector3Equal(hitPoint,
									     m_BrushTool.lastPlacedObjectInfo.raycastInfo.point)))
								{
									...
									if (placedObjectInfo != null)
									{
										...
										if (brush.settings.alongBrushStroke &&
										    m_BrushTool.lastPlacedObjectInfo != null)
										{
										...
										if (m_BrushTool.lastPlacedObjectInfo != null)
										{
											BrushModeFinishPlaceObject(m_BrushTool
												.lastPlacedObjectInfo);
											m_BrushTool.lastPlacedObjectInfo = null;
										}
							...
							else
							{
								if (isTwoPoints && !Utility.IsVector3Equal(hitPoint, lastHitPoint))
								{
									float brushRadius = brush.settings.brushRadius;
									float brushSpacing =
										Mathf.Max(0.01f, brush.settings.brushSpacing);
									...
									m_BrushTool.strokeDirection =
										(hitPoint - m_BrushTool.strokeDirectionRefPoint).normalized;
									...
									if (m_BrushTool.dragDistance + moveLenght >= brushSpacing)
									{
										...
										Vector3 randomPoint = drawPoint +
										                      Vector3.ProjectOnPlane(
											                      UnityEngine.Random.onUnitSphere,
											                      hitNormal) * brushRadius;
						...
						case KeyCode.F:
							...
							if (IsModifierDown(EventModifiers.None) && m_CurrentRaycast.isHit)
							{
								if (brush != null && !brush.settings.gridEnabled)
									...
									SceneView.lastActiveSceneView.LookAt(m_CurrentRaycast.point,
										SceneView.lastActiveSceneView.rotation,
										Mathf.Max(brush.settings.gridStep.x,
											brush.settings.gridStep.y) * 10f);
								e.Use();
							}
			...
			switch (e.GetTypeForControl(controlID))
			{
				case EventType.MouseDown:
					if (e.button == 0 && !e.alt && brush != null && !hasMultipleSelectedBrushes)
					{
						...
						if (m_CurrentRaycast.isHitTargetLayer)
						{
							...
							GetOrientation(m_CurrentRaycast.normal, brush.settings.orientationMode,
								out m_PinTool.upwards, out m_PinTool.right, out m_PinTool.forward);


							if (brush.settings.pinFixedRotation)
								OrientObject(m_PinTool.placedObjectInfo,
							...
							m_PinTool.scaleFactor =
								GetObjectScaleFactor(m_PinTool.placedObjectInfo.pivotObject,
									m_CurrentRaycast);
							...
							if (brush.settings.pinFixedScale)
							{
								m_PinTool.placedObjectInfo.pivotObject.transform.localScale =
									brush.settings.pinFixedScaleValue;
							}
							else
								m_PinTool.placedObjectInfo.pivotObject.transform.localScale =
				...
				case EventType.MouseDrag:
					if (GUIUtility.hotControl == controlID && e.button == 0 &&
					    m_PinTool.placedObjectInfo != null)
					{
						if (m_PinTool.IntersectsHitPlane(
							    HandleUtility.GUIPointToWorldRay(e.mousePosition),
							    out m_PinTool.point))
						{
							Vector3 vector = m_PinTool.point -
							...
							bool snapScale =
								e.shift ? !m_Settings.pinSnapScale : m_Settings.pinSnapScale;
							...
							{
								scale = Mathf.Round(scale / m_Settings.pinSnapScaleValue) *
							...
							PivotMode pivotMode = brush.settings
								.multibrushSlots[m_PinTool.placedObjectInfo.prefabSlot].pivotMode;
							...
							Vector3 pivot = GetObjectPivot(m_PinTool.placedObjectInfo.pivotObject,
								pivotMode);

							m_PinTool.placedObjectInfo.pivotObject.transform.position =
								m_PinTool.placedObjectInfo.raycastInfo.point
								- (pivot - m_PinTool.placedObjectInfo.pivotObject.transform
									.position)
				...
				case EventType.MouseUp:
					if (GUIUtility.hotControl == controlID && e.button == 0)
					{
						if (m_PinTool.placedObjectInfo != null)
						{
							...
							{
								Vector2 placeSrceenPoint =
									HandleUtility.WorldToGUIPoint(m_PinTool.placedObjectInfo
										.raycastInfo.point);

								if ((e.mousePosition - placeSrceenPoint).magnitude < 5f)
								{
									GameObject.DestroyImmediate(m_PinTool.placedObjectInfo
										.gameObject);
								}
							}
		...
		void DoPlaceTool()
		{
			...
			switch (e.GetTypeForControl(controlID))
			{
				case EventType.MouseDown:
					...
					{
						GetOrientation(m_CurrentRaycast.normal, brush.settings.orientationMode,
							out m_PlaceTool.upwards, out m_PlaceTool.right,
				...
				case EventType.Repaint:
					...
					{
						if (m_PlaceTool.placedObjectInfo != null &&
						    (brush == null || hasMultipleSelectedBrushes ||
						...
						if (brush != null && !hasMultipleSelectedBrushes)
						{
							    ...
							    m_PlaceTool.raycastInfo.isHit)
							{
								...
								m_PlaceTool.placedObjectInfo =
									PlaceObject(m_PlaceTool.raycastInfo, brush);
								brush.EndStroke();
							}
						}
						...
						if (m_PlaceTool.placedObjectInfo != null)
						{
							...
							OrientObject(m_PlaceTool.placedObjectInfo,
								brush.settings.placeEulerAngles);
							m_PlaceTool.placedObjectInfo.pivotObject.transform.localScale =
								new Vector3(brush.settings.placeScale, brush.settings.placeScale,
									brush.settings.placeScale);

							Vector3 pivot = GetObjectPivot(m_PlaceTool.placedObjectInfo.pivotObject,
								brush.settings
							...
							m_PlaceTool.placedObjectInfo.pivotObject.transform.position =
								...
								- (pivot - m_PlaceTool.placedObjectInfo.pivotObject.transform
									.position)
								+ brush.settings.surfaceOffset *
							...
							m_PlaceTool.placedObjectInfo.pivotObject.transform.position =
								Utility.RoundVector(
									m_PlaceTool.placedObjectInfo.pivotObject.transform.position, 3);
						}
					}
					...
					switch (e.keyCode)
					{
						case KeyCode.F:
							...
							if (IsModifierDown(EventModifiers.None) && m_CurrentRaycast.isHit)
							{
								...
								{
									Bounds bounds;
									GetObjectWorldBounds(m_PlaceTool.placedObjectInfo.pivotObject,
										out bounds);
		...
		void ThrowObject(RaycastInfo raycastInfo, Brush brush)
		{
			...
			if (m_Settings.overwritePrefabLayer)
			{
				Utility.ForAllInHierarchy(gameObject,
					go => { go.layer = m_Settings.prefabPlaceLayer; });
			}
			...
			switch (e.GetTypeForControl(controlID))
			{
				...
				case EventType.MouseDrag:
					    ...
					    !hasMultipleSelectedBrushes)
					{
						...
						if (!m_ThrowTool.raycastInfo.isHit)
						{
							if (!m_ThrowTool.prevRaycast.IntersectsHitPlane(
								    m_ThrowTool.raycastInfo.ray, out hitPoint))
								isTwoPoints = false;
						}

						if (!m_ThrowTool.prevRaycast.isHit)
						{
							if (!m_ThrowTool.raycastInfo.IntersectsHitPlane(
								    m_ThrowTool.prevRaycast.ray, out lastHitPoint))
								isTwoPoints = false;
						}
						...
						if (m_ThrowTool.dragDistance + moveLenght >= throwSpacing)
						{
							...
							Vector3 randomPoint = drawPoint +
							                      Vector3.ProjectOnPlane(
								                      UnityEngine.Random.onUnitSphere, hitNormal) *
							                      throwRadius;
			...
			switch (eventType)
			{
				...
				case EventType.MouseDrag:
							...
							m_Settings.GetActiveTab().brushes.ForEach((b) =>
							{
								if (b.selected)
								{
									if (b.settings.multibrushEnabled)
									{
										...
										{
											if (b.prefabSlots[i].gameObject != null &&
											    b.settings.multibrushSlots[i].enabled)
											{
												m_EraseTool.prefabList.Add(b.prefabSlots[i]
													.gameObject);
											}
										}
									}
					...
					if (GUIUtility.hotControl == controlID)
					{
						...
						m_Octree.IntersectSphere(m_CurrentRaycast.point,
							m_Settings.eraseBrushRadius, (go) =>
							{
								if (go == null)
									return true;
								...
								{
									if (((1 << prefabRoot.layer) & m_Settings.eraseLayers.value) !=
									    0)
										DestroyObjectImmediate(prefabRoot);
								}
								else if (m_EraseTool.prefabList.Contains(
									         GetCorrespondingObjectFromSource(prefabRoot) as
										         GameObject))
								{
									DestroyObjectImmediate(prefabRoot);
								}
			...
			switch (eventType)
			{
				...
				case EventType.MouseDrag:
									...
									if (b.settings.multibrushEnabled)
									{
										for (int i = 0; i < b.prefabSlots.Length; i++)
										{
											if (b.prefabSlots[i].gameObject != null &&
											    b.settings.multibrushSlots[i].enabled)
											{
												m_SelectionTool.prefabList.Add(b.prefabSlots[i]
													.gameObject);
											}
										}
									}
					...
					if (GUIUtility.hotControl == controlID)
					{
						...
						m_Octree.IntersectSphere(m_CurrentRaycast.point,
							m_Settings.selectBrushRadius, (go) =>
							{
								if (go == null)
									return true;
								...
								{
									if (((1 << prefabRoot.layer) & m_Settings.selectLayers.value) ==
									    0)
										return true;
								}
								else if (!m_SelectionTool.prefabList.Contains(
									         GetCorrespondingObjectFromSource(prefabRoot) as
										         GameObject))
									return true;
			...
			switch (eventType)
			{
				...
				case EventType.MouseDrag:
									...
									if (b.settings.multibrushEnabled)
									{
										for (int i = 0; i < b.prefabSlots.Length; i++)
										{
											if (b.prefabSlots[i].gameObject != null &&
											    b.settings.multibrushSlots[i].enabled)
											{
												m_ModifyTool.prefabList.Add(b.prefabSlots[i]
													.gameObject);
											}
										}
									}
					    ...
					    GUIUtility.hotControl == controlID)
					{
						...
						m_Octree.IntersectSphere(m_CurrentRaycast.point,
							m_Settings.modifyBrushRadius, (go) =>
							{
								if (go == null)
									return true;
								...
								{
									if (((1 << prefabRoot.layer) & m_Settings.modifyLayers.value) ==
									    0)
										return true;
								}
								else
								{
									if (!m_ModifyTool.prefabList.Contains(
										    GetCorrespondingObjectFromSource(prefabRoot) as
											    GameObject))
										return true;
								}

								if (!m_ModifyTool.modifiedObjects.TryGetValue(prefabRoot,
									    out modifyInfo))
								{
									modifyInfo.pivot = GetObjectPivot(prefabRoot,
									...
									m_Octree.AddDynamicObject(prefabRoot,
								...
								if (modifyInfo.lastUpdate != m_ModifyTool.updateTicks)
								{
									...
									float falloff = 2f - distance /
										Mathf.Max(m_Settings.modifyBrushRadius, 0.001f);
									float strength = m_Settings.modifyStrength * moveLenght;

									modifyInfo.currentScale += m_Settings.modifyScale * strength *
									                           falloff * modifyInfo.randomScale *
									                           0.005f;
									...
									if (m_Settings.modifyRandomRotation)
									{
										qRotation = transform.rotation *
										            Quaternion.Euler(modifyInfo.randomRotation *
											            falloff * strength * 0.1f);
									}
									else
									{
										qRotation = transform.rotation *
										            Quaternion.Euler(
											            m_Settings.modifyRotationValues * falloff *
											            strength * 0.1f);
									}

									Quaternion qPosition = qRotation *
									                       Quaternion.Inverse(modifyInfo
										                       .initialRotation);

									transform.rotation = qRotation;
									transform.position = modifyInfo.pivot +
									                     modifyInfo.currentScale *
									                     (qPosition * (modifyInfo.initialPosition -
										                     modifyInfo.pivot));
									transform.localScale = modifyInfo.initialScale *
									                       modifyInfo.currentScale;
								}
				...
				case EventType.MouseDrag:
					...
					if (eventType == EventType.MouseDown && e.button == 0 && !e.alt)
					{
						...
						for (int i = 0; i < selectedObjects.Length; i++)
						{
							...
							objectInfo.pivot =
								GetObjectPivot(gameObject, m_Settings.orientPivotMode);
					...
					if (e.button == 0 && !e.alt && GUIUtility.hotControl == controlID)
					{
						if (m_CurrentRaycast.isHit)
						{
							foreach (OrientTool.ObjectInfo objectInfo in m_OrientTool.objects)
							{
								...
								if (m_Settings.orientSameDirection)
									forward = (m_CurrentRaycast.point - m_OrientTool.objectsCenter)
										.normalized;
								else
									forward = (m_CurrentRaycast.point - objectInfo.pivot)
										.normalized;
								...
								if (m_Settings.orientLockUp)
									forward = Vector3.ProjectOnPlane(forward, objectInfo.initialUp)
										.normalized;

								Quaternion quaternion =
									Quaternion.LookRotation(forward, objectInfo.initialUp) *
									Quaternion.Euler(m_Settings.orientRotation);
								Quaternion qPos = quaternion *
								                  Quaternion.Inverse(objectInfo.initialRotation);

								Undo.RegisterCompleteObjectUndo(transform, "Orient Object(s)");

								transform.position = objectInfo.pivot +
								                     qPos * (objectInfo.initialPosition -
								                             objectInfo.pivot);
		...
		void MoveToolReloadSelection()
		{
			...
			for (int i = 0; i < selectedObjects.Length; i++)
			{
				...
				{
					m_MoveTool.initialObjectsCenter = (m_MoveTool.initialObjectsCenter +
					                                   GetObjectPivot(gameObject,
						                                   PivotMode.WorldBoundsCenter)) * 0.5f;
				}
			...
			switch (eventType)
			{
				...
				case EventType.MouseDrag:
					    ...
					    m_MoveTool.objects.Count > 0)
					{
						...
						if (PointLineDistance(m_MoveTool.objectsCenter, ray.origin, ray.direction) <
						    m_MoveTool.handleDiskSize)
						{
							if (m_CurrentRaycast.isHit)
							{
							...
							{
								Plane diskHandlePlane = new Plane(-ray.direction,
					...
					if (e.button == 0 && !e.alt && GUIUtility.hotControl == controlID)
					{
						...
						for (int i = 0; i < objCount; i++)
						{
							...
							RaycastInfo raycast;
							if (Raycast(ray, out raycast, m_Settings.paintLayers.value,
								    m_Settings.ignoreLayers.value))
							{
							...
							{
								if (m_MoveTool.lastHitRaycast.isHit &&
								    m_MoveTool.lastHitRaycast.IntersectsHitPlane(ray, out hitPoint))
								{
									newpos = hitPoint;
							...
							newpos = newpos + surfaceNormal.normalized *
				...
				case EventType.Layout:
					if (m_MoveTool.objects.Count > 0)
						HandleUtility.AddControl(controlID,
							HandleUtility.DistanceToDisc(m_MoveTool.objectsCenter,
		...
		Ray WorldPointToRay(Vector3 worldSpacePoint)
		{
			if (Camera.current == null)
				return new Ray();
			return new Ray(Camera.current.transform.position,
				(worldSpacePoint - Camera.current.transform.position).normalized);
		}

		#endregion // Scene UI

		#region MapEditor Link

		public void Active()
		{
			if (MapRender.instance.CheckMapValid())
				ActivePainter();
		}
		...
		public void OnPrefabPainterGUI()
		{
			if (MapRender.instance.map != null)
				OnGUI();
		}

		public void ClearMapObjects()
		{
			MapRender.instance.ClearMapObjects();
			Reset();
		}
		public void LoadMapObjects()
		{
			Reset();
			MapRender.instance.LoadMapObjects();
		}
		public void SaveMapObjects()
		{
			MapRender.instance.SaveMapObjects();
		}
		
		public void ExportMapObjects()
		{
			MapRender.instance.ExportMapObjects(MapRender.instance.parentForPrefabs.transform,
				MapRender.instance.map.RuntimeMapDataPath,
				MapRender.instance.map.RuntimeMapPrefabPath);
		}

		// public void GetAllPrefabTabs()
		// {
		// 	if (m_Settings != null)
		// 		m_Settings.GetTab();
		// }
		...
		public void AddObject(GameObject go, bool useAdditionalVertexStreams = false)
		{
			m_Octree.AddObject(go, useAdditionalVertexStreams);
			AddGoToGrid(go.transform);
		}

		public void RemoveObject(Transform go)
		{
			RemoveGoFromGrid(go.transform);
		}
