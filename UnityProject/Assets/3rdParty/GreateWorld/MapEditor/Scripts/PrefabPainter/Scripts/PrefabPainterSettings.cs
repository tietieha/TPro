...
namespace GEngine.MapEditor
{
	...
	public class BrushSettings
	{
		...
		public OverlapCheckMode brushOverlapCheckMode = OverlapCheckMode.Bounds;
		public float brushOverlapDistance;
		public OverlapCheckObjects brushOverlapCheckObjects = OverlapCheckObjects.SamePlaceLayer;
		...
		public float scatterMinScale = 1;
		public float scatterMaxScale = 1;
		...
		{
			multibrushSlots[slot].enabled = false;
		...
		public void CopyFrom(BrushSettings other)
		{
			...
			for (int i = 0;
			     i < Mathf.Min(multibrushSlots.Length, other.multibrushSlots.Length);
			     i++)
			{
		...
		public Vector3 GetScale()
		{
			...
			if (scaleMode == AxisMode.Uniform)
			{
				float scaleValue = scaleUniformMin + randomVector.x *
					(scaleUniformMax - scaleUniformMin);
				scale = new Vector3(scaleValue, scaleValue, scaleValue);
			}
			else
			{
				scale = new Vector3(
					scalePerAxisMin.x + randomVector.x * (scalePerAxisMax.x - scalePerAxisMin.x),
					scalePerAxisMin.y + randomVector.y * (scalePerAxisMax.y - scalePerAxisMin.y),
					scalePerAxisMin.z + randomVector.z * (scalePerAxisMax.z - scalePerAxisMin.z));
			}
	...
	public class Brush
	{
				...
				switch (settings.multibrushMode)
				{
					case MultibrushMode.Random:
					{
						...
						for (int i = 0; i < prefabSlots.Length; i++)
						{
							if (prefabSlots[i].gameObject != null &&
							    settings.multibrushSlots[i].enabled)
							{
					...
					case MultibrushMode.Pattern:
					{
						...
						if (lastPatternString != settings.multibrushPattern)
						{
							...
							int value = 0;
							char[] separator = { ' ', ';', ',' };
							string[] patternKeys = lastPatternString.Split(separator,
						...
						{
							patternCurrentPosition = Mathf.Clamp(patternCurrentPosition, 0,
								parsedPattern.Count - 1);
						}
					}
						break;
				}
		...
		public void AssignPrefabToEmptySlot(GameObject gameObject)
		{
			for (int prefabSlot = 0; prefabSlot < prefabSlots.Length; prefabSlot++)
			{
				...
				{
					Undo.RegisterCompleteObjectUndo(PrefabPainterSettings.current,
						"PP: Assign Prefab");
					prefabSlots[prefabSlot].gameObject = gameObject;
					return;
				}
	...
	public class Tab
	{
		...
		public void InsertSelectedBrushes(int index, bool after)
		{
			...
			{
				if (brush.selected) selectedBrushes.Add(brush);
			});
			...
			if (selectedBrushes.Count > 0)
			{
				...
				brushes.InsertRange(brushes.IndexOf(null),
					selectedBrushes); // insert selected brushes after null marker
		...
		public void ResetSelectedBrushes()
		{
			...
			brushes.ForEach((brush) =>
			{
				if (brush.selected) brush.settings.Reset();
			});
		}
		...
		public void SetSelectedBrushesTag(ColorTag tag)
		{
			...
			brushes.ForEach((brush) =>
			{
				if (brush.selected) brush.colorTag = tag;
			});
		}
	...
	public class PrefabPainterSettings : ScriptableObject
	{
		...
		public void ResetScatterSettings()
		{
		...
		public void ClipboardCopyBrushes()
		{
			if (GetActiveTab().HasSelectedBrushes())
			{
				...
				GetActiveTab().brushes.ForEach((b) =>
				{
					if (b.selected) copied.Add(new Brush(b));
				});
