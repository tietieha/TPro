...
namespace U3DExtends
{
    public class PrefabWin : EditorWindow
    {
        ...
        public float SizePercent
        {
            ...
            set
            {
                if (mSizePercent != value)
                {
                    mReset = true;
                    mSizePercent = value;
                    mCellSize = Mathf.FloorToInt(200 * SizePercent + 10);
        ...
        string saveKey
        {
            get { return $"{UIEditorConfigure.TemplateListCacheFile}-{mTab}"; }
        }
        ...
        void OnEnable()
        {
            ...
            mStyle.normal.textColor = UnityEditor.EditorGUIUtility.isProSkin
        ...
        void PingItem(object obj)
        {
            ...
            if (index < mItems.size && index > -1)
            {
                ...
                UnityEditor.Selection.activeObject = item.prefab;
                UnityEditor.EditorGUIUtility.PingObject(item.prefab);
            }
        }
        ...
        void Load()
        {
            ...
            if (!File.Exists(saveKey)) return;
            string[] data = File.ReadAllLines(saveKey);
        ...
        void GeneratePreview(Item item, bool isReCreate = true)
        {
            ...
            {
                string preview_path = UIEditorConfigure.PreviewCachePath + item.prefab.name + ".png";
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Toggle(newTab == 1, "界面模板", "ButtonRight")) newTab = 1;
            ...
            for (int i = 0; i < mItems.size;)
            {
                ...
                if (!has)
                {
                    if (string.IsNullOrEmpty(searchFilter) ||
                        mItems[i].prefab.name.IndexOf(searchFilter, System.StringComparison.CurrentCultureIgnoreCase) !=
                        -1)
                        indices.Add(i);
                }
                ...
                for (int i = 0; i < indices.size; ++i)
                {
                    ...
                    if (GUI.Button(rect, mContent, "Button"))
                    {
                        if (currentEvent.button == 0)
                        {
                            ...
                            {
                                UnityEditor.Selection.activeObject = ent.prefab;
                                UnityEditor.EditorGUIUtility.PingObject(ent.prefab);
                            }
                        }
                        else if (currentEvent.button == 1)
                        {
                            ContextMenu.AddItemWithArge("Ping", false, PingItem, index);
                            ContextMenu.AddItemWithArge("Delete", false, RemoveItem, index);
                            ContextMenu.AddItemWithArge("Update Preview", false, UpdatePreView, index);
                            ContextMenu.Show();
                        }
                    }
                    ...
                    if (ent != null)
                    {
                        ...
                        if (ent.tex != null)
                        {
                            ...
                            labelPos.y = inner.yMax - labelPos.height;
                            labelStyle.fontSize = (int)(_labelDefaultFontSize * SizePercent);
                            labelStyle.alignment = TextAnchor.LowerCenter;
                            {
                                EditorGUI.DrawRect(labelPos, Color.black);
            ...
            //else if (mTab != 0)
            {
                GUILayout.BeginHorizontal();
                {
                    string after = EditorGUILayout.TextField("", searchFilter, "SearchTextField",
                        GUILayout.Width(Screen.width - 20f));
