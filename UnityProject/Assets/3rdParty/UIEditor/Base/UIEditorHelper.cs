...
namespace U3DExtends
{
    public static class UIEditorHelper
    {
        ...
        [MenuItem("Edit/Copy Names " + UIEditorConfigure.ShortCut.CopyNodesName, false, 2)]
        ...
        {
            GameObject testUI = GameObject.Find(UIEditorConfigure.UITestNodeName);
            if (!testUI)
            {
                testUI = new GameObject(UIEditorConfigure.UITestNodeName, typeof(RectTransform));
                RectTransform trans = testUI.GetComponent<RectTransform>();
                trans.position = UIEditorConfigure.UITestNodePos;
                trans.sizeDelta = UIEditorConfigure.UITestNodeSize;
        ...
        static public Transform GetContainerUnderMouse(Vector3 mouse_abs_pos, GameObject ignore_obj = null)
        {
            ...
            foreach (var item in containers)
            {
                ...
                if (trans != null)
                {
                    ...
                    if (mouse_abs_pos.x >= corners[0].x && mouse_abs_pos.y <= corners[1].y &&
                        mouse_abs_pos.x <= corners[2].x && mouse_abs_pos.y >= corners[3].y)
                    {
                        list.Add(trans);
                    }
                }
            ...
            if (list.Count <= 0)
                return null;
            list.Sort((RectTransform a, RectTransform b) =>
                {
        ...
        public static GameObject CreatNewLayout(bool isNeedLayout = true)
        {
            ...
            string file_path = Path.Combine(UIEditorConfigure.ResAssetsPath, "Canvas.prefab");
            // file_path = FileUtil.GetProjectRelativePath(file_path);
            GameObject layout_prefab = AssetDatabase.LoadAssetAtPath(file_path, typeof(GameObject)) as GameObject;
        ...
        {
            string file_path = Path.Combine(UIEditorConfigure.ResAssetsPath, "Decorate.prefab");
            // file_path = FileUtil.GetProjectRelativePath(file_path);
            GameObject decorate_prefab =
                UnityEditor.AssetDatabase.LoadAssetAtPath(file_path, typeof(UnityEngine.Object)) as GameObject;
        ...
        public static void CreateDecorate()
        {
            if (Selection.activeTransform != null)
            {
                ...
                if (canvas != null)
                {
                    ...
                    if (UIEditorConfigure.OpenSelectPicDialogWhenAddDecorate)
                    {
        ...
        public static void ClearAllCanvas()
        {
            ...
            if (isDeleteAll)
            {
                GameObject test = GameObject.Find(UIEditorConfigure.UITestNodeName);
                if (test != null)
                {
        ...
        public static Transform LoadLayoutByPath(string select_path)
        {
            ...
            GameObject new_view = PrefabUtility.InstantiatePrefab(prefab) as GameObject;
        ...
        public static Texture GetAssetPreview(GameObject obj)
        {
            ...
            RenderTexture texture = new RenderTexture(256, 256, 0, RenderTextureFormat.Default);
            ...
            RenderTexture tex = new RenderTexture(256, 256, 0, RenderTextureFormat.Default);
        ...
        public static Bounds GetBounds(GameObject obj)
        {
            ...
            if (renders.Length > 0)
            {
                ...
                {
                    if (renders[i].gameObject.name.Contains("BG"))
                        continue;
            ...
            else
            {
                ...
                if (rectTrans.rect.width == 0 || rectTrans.rect.height == 0)
                {
                    // Handle the case where either width or height is zero
                    float aspectRatio = 16f / 9f;
                    float width = rectTrans.rect.width == 0 ? rectTrans.rect.height * aspectRatio : rectTrans.rect.width;
                    float height = rectTrans.rect.height == 0 ? rectTrans.rect.width / aspectRatio : rectTrans.rect.height;
                    ...
                    corner[0] = rectTrans.TransformPoint(new Vector3(0, -height / 2, 0));
                    corner[1] = rectTrans.TransformPoint(new Vector3(0, height / 2, 0));
                    corner[2] = rectTrans.TransformPoint(new Vector3(width, height / 2, 0));
                    corner[3] = rectTrans.TransformPoint(new Vector3(width, -height / 2, 0));
                }
                else
                {
                    //获取节点的四个角的世界坐标，分别按顺序为左下左上，右上右下
                    rectTrans.GetWorldCorners(corner);
                }
        ...
        // 判断 RectTransform 是否在自适应模式（例如使用 ContentSizeFitter 或 LayoutGroup）
        static bool IsInAdaptiveMode(RectTransform rectTransform)
        {
            // 这里可以根据需求，检查 RectTransform 的 LayoutGroup 或 ContentSizeFitter 是否启用
            var fitter = rectTransform.GetComponent<ContentSizeFitter>();
            return fitter != null && fitter.horizontalFit == ContentSizeFitter.FitMode.Unconstrained;
        }
        ...
        public static void SaveAnotherLayout(Canvas layout, Transform child)
        {
            ...
            if (UIEditorConfigure.IsShowDialogWhenSaveLayout)
        ...
        public static void SaveLayout(GameObject o, bool isQuiet)
        {
            ...
            if (real_layout != null)
            {
                    ...
                    cur_prefab_type == PrefabType.DisconnectedPrefabInstance)
                {
                    ...
                    //刷新
                    AssetDatabase.Refresh();
                    if (UIEditorConfigure.IsShowDialogWhenSaveLayout && !isQuiet)
                        EditorUtility.DisplayDialog("Tip", "Save Succeed!", "Ok");
        ...
        static public void AddImageComponent()
        {
            ...
            Image img = Undo.AddComponent<Image>(Selection.activeGameObject);
        ...
        static public void AddHorizontalLayoutComponent()
        {
            ...
            HorizontalLayoutGroup layout = Undo.AddComponent<HorizontalLayoutGroup>(Selection.activeGameObject);
        ...
        static public void AddVerticalLayoutComponent()
        {
            if (Selection.activeGameObject == null)
                return;
            VerticalLayoutGroup layout = Undo.AddComponent<VerticalLayoutGroup>(Selection.activeGameObject);
            layout.childForceExpandWidth = false;
            layout.childForceExpandHeight = false;
            layout.childControlWidth = false;
            layout.childControlHeight = false;
        ...
        static public void AddGridLayoutGroupComponent()
        {
            if (Selection.activeGameObject == null)
                return;
            GridLayoutGroup layout = Undo.AddComponent<GridLayoutGroup>(Selection.activeGameObject);
        }
        ...
        static public void CreateEmptyObj()
        {
            if (Selection.activeGameObject == null)
                return;
            GameObject go = new GameObject(CommonHelper.GenerateUniqueName(Selection.activeGameObject, "GameObject"),
                typeof(RectTransform));
            go.transform.SetParent(GetGoodContainer(Selection.activeTransform), false);
            Undo.RegisterCreatedObjectUndo(go, "CreateEmptyObj");
        ...
        static public void CreateImageObj()
        {
            ...
            {
                GameObject go = new GameObject(CommonHelper.GenerateUniqueName(Selection.activeGameObject, "Image"),
                    typeof(Image));
        ...
        static public void CreateRawImageObj()
        {
            if (Selection.activeTransform && Selection.activeTransform.GetComponentInParent<Canvas>())
            {
                GameObject go = new GameObject(CommonHelper.GenerateUniqueName(Selection.activeGameObject, "RawImage"),
                    typeof(RawImage));
                go.GetComponent<RawImage>().raycastTarget = false;
                go.transform.SetParent(GetGoodContainer(Selection.activeTransform), false);
                Undo.RegisterCreatedObjectUndo(go, "CreateRawImageObj");
                Selection.activeGameObject = go;
            }
        }
        ...
        static public void CreateButtonObj()
        {
            if (Selection.activeTransform && Selection.activeTransform.GetComponentInParent<Canvas>())
            {
                ...
                bool isOk = EditorApplication.ExecuteMenuItem("GameObject/UI/Legacy/Button");
                if (isOk)
                {
        ...
        static public void CreateTextObj()
        {
            if (Selection.activeTransform && Selection.activeTransform.GetComponentInParent<Canvas>())
            {
                GameObject go = new GameObject(CommonHelper.GenerateUniqueName(Selection.activeGameObject, "Text"),
