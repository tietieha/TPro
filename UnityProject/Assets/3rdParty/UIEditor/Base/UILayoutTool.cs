...
namespace U3DExtends
{
    public class UILayoutTool : MonoBehaviour
    {
        public static void ResortAllLayout()
        {
            ...
            if (testUI != null)
            {
                ...
                if (layouts.Length > 0)
                {
                    ...
                    Vector2 startPos = new Vector2(first_trans.sizeDelta.x * first_trans.localScale.x / 2,
                    ...
                    foreach (var item in layouts)
                    {
                        ...
                        Vector3 pos = new Vector3((rectTrans.sizeDelta.x * rectTrans.localScale.x) * col + startPos.x,
        ...
        {
            Undo.RecordObjects(Selection.gameObjects, "Show All Selected Widgets");
        ...
        {
            Undo.RecordObjects(Selection.gameObjects, "Hide All Selected Widgets");
            foreach (var item in Selection.gameObjects)
            {
    ...
    public class PriorityTool
    {
        ...
        public static void MoveToTopWidget()
        {
            ...
            if (curSelect != null)
            {
                Undo.SetSiblingIndex(curSelect, 0, "Move To Top Widget");
        ...
        public static void MoveToBottomWidget()
        {
            ...
            if (curSelect != null)
            {
                ...
                {
                    Undo.SetSiblingIndex(curSelect, child_num - 1, "Move To Bottom Widget");
        ...
        public static void MoveUpWidget()
        {
            ...
            if (curSelect != null)
            {
                ...
                if (curIndex > 0)
                {
                    Undo.SetSiblingIndex(curSelect, curIndex + 1, "Move Up Widget");
        ...
        public static void MoveDownWidget()
        {
            ...
            if (curSelect != null)
            {
                ...
                if (curIndex < child_num - 1)
                {
                    Undo.SetSiblingIndex(curSelect, curIndex + 1, "Move Down Widget");
                    // curSelect.SetSiblingIndex(curIndex + 1);
                }
            }
        }
    ...
    public class AlignTool
    {
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.transforms, "Align Horizontal Left");
            // Undo.RecordObjects(Selection.transforms, "Align Horizontal Left");
            float x = Mathf.Min(Selection.transforms.Select(trans => trans.localPosition.x).ToArray());

            for (int i = 0; i < Selection.transforms.Length; i++)
            {
                var trans = Selection.transforms[i];
                Undo.RecordObject(trans, "Per Align Horizontal Left");
                trans.localPosition = new Vector2(x, trans.localPosition.y);
                EditorUtility.SetDirty(trans); // 标记为脏
            }
        }
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.transforms, "Align Horizontal Right");
            // Undo.RecordObjects(Selection.transforms, "Align Horizontal Right");
            float x = Mathf.Max(Selection.transforms.Select(trans => trans.localPosition.x + ((RectTransform)trans).sizeDelta.x).ToArray());
            for (int i = 0; i < Selection.transforms.Length; i++)
            {
                var trans = Selection.transforms[i];
                Undo.RecordObject(trans, "Per Align Horizontal Right");
                trans.localPosition = new Vector3(x - ((RectTransform)trans).sizeDelta.x, trans.localPosition.y);
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.transforms, "Align Vertical Up");
            float y = Mathf.Max(Selection.transforms.Select(trans => trans.localPosition.y).ToArray());
            for (int i = 0; i < Selection.transforms.Length; i++)
            {
                var trans = Selection.transforms[i];
                Undo.RecordObject(trans, "Per Align Vertical Up");
                trans.localPosition = new Vector3(trans.localPosition.x, y);
                EditorUtility.SetDirty(trans);
            }
        }
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.transforms, "Align Vertical Down");
            float y = Mathf.Min(Selection.transforms.Select(trans => trans.localPosition.y - ((RectTransform)trans).sizeDelta.y).ToArray());

            for (int i = 0; i < Selection.transforms.Length; i++)
            {
                var trans = Selection.transforms[i];
                Undo.RecordObject(trans, "Per Align Vertical Down");
                trans.localPosition = new Vector3(trans.localPosition.x, y + ((RectTransform)trans).sizeDelta.y);
                EditorUtility.SetDirty(trans); // 强制标记对象为脏
            }
        }
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "Uniform Distribution In Horziontal");
            ...
            {
                Undo.RecordObject(objects[i].transform, "Per Uniform Distribution In Horziontal");
                objects[i].transform.localPosition =
                    new Vector3(firstX + i * distance, objects[i].transform.localPosition.y);
                EditorUtility.SetDirty(objects[i].transform); // 强制标记对象为脏
            }
        }
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "Uniform Distribution In Vertical");
            int count = Selection.gameObjects.Length;
            ...
            for (int i = 0; i < count; i++)
            {
                Undo.RecordObject(objects[i].transform, "Per Uniform Distribution In Vertical");
                objects[i].transform.localPosition =
                    new Vector3(objects[i].transform.localPosition.x, firstY + i * distance);
                EditorUtility.SetDirty(objects[i].transform);
            }
        }
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "Resize Max");
            ...
            {
                Undo.RecordObject(gameObject.transform, "Per Resize Max");
        ...
        {
            Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "Resize Min");
            ...
            foreach (GameObject gameObject in Selection.gameObjects)
            {
                Undo.RecordObject(gameObject.transform, "Per Resize Min");
                ((RectTransform)gameObject.transform).sizeDelta = new Vector2(width, height);
                EditorUtility.SetDirty(gameObject.transform);
            }
        }
