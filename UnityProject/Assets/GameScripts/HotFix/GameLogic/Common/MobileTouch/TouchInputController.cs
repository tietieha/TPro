...
namespace BitBenderGames
{
	...
	public class TouchInputController : MonoBehaviour
	{
		...
		private BetterList<Vector3> DragFinalMomentumVector;
		...
		public void Awake()
		{
			...
			DragFinalMomentumVector = new BetterList<Vector3>();
		...
		public void Update()
		{
				...
				if (isPinching == false)
				{
					if (wasPinchingLastFrame == false)
					{
						if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown)
						{
							if (isDragging == false)
							{
								...
								{
#if false
                                    Log.Info("touch position({0}, {1}), Delta({2}, {3}), TapCount{4}, FingerId{5}, Phase{6}",
                                        Input.touches[0].position.x, Input.touches[0].position.y,
                                        Input.touches[0].deltaPosition.x, Input.touches[0].deltaPosition.y,
                                        Input.touches[0].tapCount,
					...
					else
					{
						if (TouchWrapper.IsFingerDown == true)
						{
							...
							DragStart(dragStartPos, false, Vector3.zero);
			...
			if (isDragging && TouchWrapper.IsFingerDown && pinchToDragCurrentFrame == false)
			{
				...
				if (DragFinalMomentumVector.size > momentumSamplesCount)
				{
		...
		private void UpdatePinch()
		{
			...
			OnPinchUpdateExtended?.Invoke(new PinchUpdateData() { pinchCenter = pinchCenter, pinchDistance = pinchDistance, pinchStartDistance = pinchStartDistance, pinchAngleDelta = pinchAngleDelta, pinchAngleDeltaNormalized = pinchAngleDeltaNormalized, pinchTiltDelta = pinchTiltDelta, pinchTotalFingerMovement = totalFingerMovement });
		...
		{
			//if (Npc3DViewerController.Instance.HeroUIIsOpen())
			//{
			//	GameEntry.Event.Fire(this, EventId.LF_Drag_Start, LFDragData.Create(true, pos));
			//}
			//else
			{
				OnDragStart?.Invoke(pos, isLongTap, offset);
			}
		...
		private void DragUpdate(Vector3 pos)
		{
			if (OnDragUpdate != null)
			{
				...
				//if (Npc3DViewerController.Instance.HeroUIIsOpen())
				//{
				//	GameEntry.Event.Fire(this, EventId.LF_Drag_Update, LFDragData.Create(true, pos));
				//	return;
				//}
		...
		private void DragStop(Vector3 pos)
		{
			if (OnDragStop != null)
			{
				...
				if (DragFinalMomentumVector.size > 0)
				{
					for (int i = 0; i < DragFinalMomentumVector.size; ++i)
					{
						momentum += DragFinalMomentumVector[i];
					}
					momentum /= DragFinalMomentumVector.size;
				}

				//if (Npc3DViewerController.Instance.HeroUIIsOpen())
				//{
				//	GameEntry.Event.Fire(this, EventId.LF_Drag_End, LFDragData.Create(true, momentum));
				//}
				//else
				{
		...
		{
			isFingerDown = false;
			OnFingerUp?.Invoke(lastFinger0DownPos);
		}
