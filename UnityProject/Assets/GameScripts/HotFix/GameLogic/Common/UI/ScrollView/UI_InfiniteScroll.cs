    ...
    public class UI_InfiniteScroll : MonoBehaviour
    {
        ...
        public void Init()
        {
            if (GetComponent<ScrollRect>() != null)
            {
                ...
                {
                    Log.Error("UI_InfiniteScroll doesn't support scrolling in both directions, please choose one direction (horizontal or vertical)");
                }
            ...
            {
                Log.Error("UI_InfiniteScroll => No ScrollRect component found");
            }
        }
        ...
        void DisableGridComponents()
        {
            ...
            if (_isHorizontal && items.Count > 1)
            {
        ...
        public void OnScroll(Vector2 pos)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (_isHorizontal)
                {
                    if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).x > _disableMarginX - _scrollRect.content.sizeDelta.x / 2)
                    {
                    ...
                    else if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).x < -_disableMarginX - _scrollRect.content.sizeDelta.x / 2)
                    {
                        _newAnchoredPosition = items[i].anchoredPosition;
                        _newAnchoredPosition.x += _itemCount * _recordOffsetX;
                        items[i].anchoredPosition = _newAnchoredPosition;
                ...
                if (_isVertical)
                {
                    if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).y > _disableMarginY - _scrollRect.content.sizeDelta.y / 2)
                    {
                        _newAnchoredPosition = items[i].anchoredPosition;
                    ...
                    else if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).y < -_disableMarginY - _scrollRect.content.sizeDelta.y / 2)
                    {
                        _newAnchoredPosition = items[i].anchoredPosition;
                        _newAnchoredPosition.y += _itemCount * _recordOffsetY;
                        items[i].anchoredPosition = _newAnchoredPosition;
        ...
        public void Clear()
        {
            if(_scrollRect) _scrollRect.onValueChanged.RemoveListener(OnScroll);
            items.Clear();
