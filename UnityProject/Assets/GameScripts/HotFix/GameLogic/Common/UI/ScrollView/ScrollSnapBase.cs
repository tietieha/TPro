...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        [Tooltip ("If hide button when button disable")] [SerializeField]
        public bool HideButtonWhenDisable = false;
        ...
        [Tooltip("不允许多次执行OnEnable")]
        public bool DontAllowReOnEnable = false;

        protected bool _hasEnable = false;
        
        public void ResetEnableStatus ()
        {
            _hasEnable = false;
        }
        ...
        public void InitialiseChildObjects()
        {
            ...
            if (ChildObjects != null && ChildObjects.Length > 0)
            {
                ...
                {
                    Log.Error("ScrollRect Content has children, this is not supported when using managed Child Objects\n Either remove the ScrollRect Content children or clear the ChildObjects array");
                    return;
                }
        ...
        internal void InitialiseChildObjectsFromScene()
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                {
                    ChildObjects[i].SetActiveEx(false);
                }
            }
        }
        ...
        public void SetEnable(bool isEnable)
        {
            ...
            _scroll_rect.enabled = isEnable;
            GetComponent<UI_InfiniteScroll>().enabled = isEnable;
        }
        ...
        public void UpdateVisible()
        {
            ...
            for (int i = CurrentPage - _bottomItem; i < CurrentPage + _topItem; i++)
            {
                try
                {
                    ChildObjects[i].SetActiveEx(true);
                }
                catch
                {
                    Log.Info($"Failed to SetActiveEx child [{i}]");
                }
            }

            //Deactivate items out of visibility at the bottom of the ScrollRect Mask (only on scroll)
            if (_currentPage > _halfNoVisibleItems) ChildObjects[CurrentPage - _bottomItem].SetActiveEx(false);
            //Deactivate items out of visibility at the top of the ScrollRect Mask (only on scroll)
            if (_screensContainer.childCount - _currentPage > _topItem) ChildObjects[CurrentPage + _topItem].SetActiveEx(false);
            
            // 补充一点逻辑
            // 这个本来是通过 MaskArea 的大小计算当前显示到什么位置，然后隐藏掉 MaskArea 之外的，但是上面的逻辑有漏洞，没有考虑到循环列表首尾相接的情况
            // 这里用一个简单的办法处理，如果列表元素数量大于1，那么在显示第一个物品时强制关掉最后一个，如果显示最后一个物品时，强制关掉第一个
            // 但是这样处理也是有一个漏洞，但是目前这个插件只有礼包有，礼包一页就一个元素，所以没问题
            if (ChildObjects.Length > 1)
            {
                if(ChildObjects.Length - 1 == _currentPage)
                    ChildObjects[0].SetActiveEx (false);
                if(_currentPage == 0)
                    ChildObjects[ChildObjects.Length - 1].SetActiveEx (false);
            }
        }
        ...
        {
            if(_pointerDown) return; //拖动的时候不跳转
        ...
        private void ToggleNavigationButtons(int targetScreen)
        {
            ...
            if (!_isInfinite)
            {
                if (PrevButton)
                {
                    var enable = targetScreen > 0;
                    
                    if (HideButtonWhenDisable)
                    {
                        if(PrevButton.activeSelf != enable)
                            PrevButton.SetActiveEx (enable);
                    }
                    else
                    {
                        PrevButton.GetComponent<Button>().interactable = enable;
                    }
                ...
                if (NextButton)
                {
                    var enable = targetScreen < _screensContainer.transform.childCount - 1;
                    if (HideButtonWhenDisable)
                    {
                        if(NextButton.activeSelf != enable)
                            NextButton.SetActiveEx (enable);
                    }
                    else
                    {
                        NextButton.GetComponent<Button>().interactable = enable;
                    }
                }
            }
        }
        ...
        private void OnValidate()
        {
            ...
            {
                Log.Error("ScrollRect has to have a direction, please select either Horizontal OR Vertical with the appropriate control.");
            }
            if (_scroll_rect.horizontal && _scroll_rect.vertical)
            {
                Log.Error("ScrollRect has to be unidirectional, only use either Horizontal or Vertical on the ScrollRect, NOT both.");
            }
            ...
            {
                Log.Error($"[{gameObject.name}]When using procedural children with a ScrollSnap (Adding Prefab ChildObjects) and the Infinite Scroll component\nYou must set the 'InitByUser' option to true, to enable late initialising");
            }
        }
        ...
        private void OnDestroy()
        {
            if (GetComponent<UI_InfiniteScroll>() != null)
            {
                GetComponent<UI_InfiniteScroll>().Clear();
            }
        }
    }
}
