...
namespace Mopsicus.Plugins {
    ...
    public class MobileInput : MonoBehaviour, IPlugin {
        ...
        public void OnData (JsonObject data) {
            Log.Info(string.Format ("{0} plugin OnData: {1}", GetType ().Name, data.ToJsonPrettyPrintString ()));
            _data = data;
            try {
                ...
                switch (code) {
                    case KEYBOARD_ACTION:
                        ...
                        int height = (int) (response["height"] * (float) Screen.height);
                ...
                Log.Error($"{GetType().Name} plugin OnData error: {e.Message}");
            }
        }
        ...
        public void OnError (JsonObject data) {
            Log.Error($"{GetType().Name} plugin OnError: {data.ToJsonPrettyPrintString()}");
            _error = data;
            _error = null;
        }
...
#if UNITY_EDITOR
            Log.Info("MobileInput execute " + json);
#elif UNITY_ANDROID
