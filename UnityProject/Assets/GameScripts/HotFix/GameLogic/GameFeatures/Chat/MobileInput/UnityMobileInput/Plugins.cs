...
namespace Mopsicus.Plugins {
	...
	public class Plugins : MonoBehaviour {
        ...
        void OnDataReceive (string data) {
            Log.Info("Plugins receive data: {0} ", data);
            try {
                ...
                if (_plugins.ContainsKey (info["name"])) {
                    ...
                    //GameFramework.Log.Error(string.Format ("{0} plugin does not exists", info["name"]));
                }
            } catch (Exception e) {
                Log.Error(string.Format ("Plugins receive error: {0}, stack: {1}", e.Message, e.StackTrace));
            }
