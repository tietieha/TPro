...
namespace Mopsicus.Plugins {
    ...
    /// </summary>
    [RequireComponent (typeof (InputField))][LuaCallCSharp]
...
//        public bool IsManualHideControl = false;
        ...
        // 失去焦点时回调 add by sunliwen 
        public Action OnLostFocus = delegate {  };
        ...
        private void Awake () {
            ...
            if ((object) _inputObject == null) {
                Log.Error(string.Format ("No found InputField for {0} MobileInput", this.name));
        ...
        //记录焦点变化前的显隐
        private bool recordVisible = false;
        /// <summary>
        /// Handler for app focus lost
        /// </summary>
        private void OnApplicationFocus (bool hasFocus) {
            Log.Info("MobileInputField 焦点变化 " + hasFocus + " visible = " + Visible);
            if (!_isMobileInputCreated) {
                return;
            }
            //如果本来是显示的就记录下
            if(this.Visible)
                recordVisible = this.Visible; 
            
            //本身显示的话根据焦点变化显隐 ，不显示也没记录所以不跟随焦点变化显隐
            if(recordVisible)
            {
        ...
        private IEnumerator InitialzieOnNextFrame () {
            ...
            _inputObject.placeholder.gameObject.SetActiveEx (false);
        ...
        {
            Log.Error($"SetUnityInputEnabled:{enabled}");
#if (UNITY_IOS || UNITY_ANDROID) && !UNITY_EDITOR
		...
		public static Rect GetScreenRectFromRectTransform (RectTransform rect) {
			...
			for (int i = 0; i < 4; i++) {
                ...
                // }
                var camera = GameObject.Find("UIRoot/GUICamera").GetComponent<Camera>();
                Vector3 screenCoord =camera.WorldToScreenPoint(corners[i]);
        ...
        private void PrepareNativeEdit () {
            ...
            {
                var placeHolder1 = _inputObject.placeholder.GetComponent<TextMeshProUGUI> ();
                _config.Placeholder = placeHolder1.text;
                _config.PlaceholderColor = placeHolder1.color;
            }
        ...
        private void OnTextChange (string text) {
            ...
            this._inputObject.SetTextEx(text);
	...
	/// <summary>
        ...
        private void CreateNativeEdit () {
            ...
            data["back_color_a"] = InvariantCultureString(0); //_config.BackgroundColor.a
        ...
        public void SetFocus (bool isFocus) {
            Log.Error($"SetFocus:{isFocus}  _isMobileInputCreated:{_isMobileInputCreated}");
#if (UNITY_IOS || UNITY_ANDROID) && !UNITY_EDITOR
            if (!_isMobileInputCreated) {
        ...
        public void SetVisible (bool isVisible) {
            Log.Error($"SetVisible:{isVisible}  _isMobileInputCreated:{_isMobileInputCreated}");
            if (!_isMobileInputCreated) {
