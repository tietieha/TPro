...
namespace RVO
{
    ...
    public class Agent
    {
        ...
        // 躲避率，值越小越不躲
        internal float avoidanceRatio = 0.5f;
        protected Vector2 newVelocity_;
        protected float _stopDis;
        ...
        public float AvoidanceRatio
        {
            get { return avoidanceRatio; }
            set { avoidanceRatio = value; }
        }
        ...
        internal void ComputeNeighbors(KdTree kdTree)
        {
            ...
            if (maxNeighbors_ > 0)
            {
                ...
                kdTree.computeAgentNeighbors(this, ref rangeSq, this.Type);
            }
        }
         ...
         */
        internal virtual void ComputeNewVelocity(float timeStep)
        {
            orcaLines_.Clear();
            ...
            for (int i = 0; i < agentNeighbors_.Count; ++i)
            {
                ...
                {
                    // Handle the case where positions are the same
                    // For example, you can skip the calculation or set a default value
                    continue;
                }
                ...
                else
                {
                    ...
                    line.direction = new Vector2(unitW.y(), -unitW.x());
                    //combinedRadius * invTimeStep 是小圆半径，乘以 unitW 可以理解为就是 relativeVelocity 到小圆圆心的连线与小圆圆周的交点，
                    //而 wLength * unitW 就是 relativeVelocity 自身，所以用前者减去后者，就是从 relativeVelocity 指到小圆圆周的向量，这不就是 u 嘛~
                    u = (combinedRadius * invTimeStep - wLength) * unitW;
                }

                //此处u是当前速度到障碍体表面最近点的向量
                line.point = velocity_ + AvoidanceRatio / (AvoidanceRatio + other.AvoidanceRatio) * u;
                orcaLines_.Add(line);
            }
         ...
         */
        internal virtual void Update(float timeStep)
        {
            ...
            ref Vector2 result)
        {
            ...
            else if (lines.Count == 0 && radius_ == 0) // 处理VirtualHero半径是0的情况
            {
                result = optVelocity;
            }
        ...
        public virtual void SetTargetId(int targetId, bool isFollow = false, bool isFirst = false)
        {
