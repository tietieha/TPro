...
namespace XLua
{
    ...
    public class LuaEnv : IDisposable
    {
        ...
        public int Memory => LuaAPI.lua_gc(rawL, LuaGCOptions.LUA_GCCOUNT, 0);
        ...
        public LuaEnv()
        {
...
#endif
            {
                ...
                //add by shushi
                LuaAPI.lua_pushstdcallcfunction(rawL, StaticLuaCallbacks.PrintError);
                if (0 != LuaAPI.xlua_setglobal(rawL, "print_error"))
                {
                    throw new Exception("call xlua_setglobal fail!");
                }

                LuaAPI.lua_pushstdcallcfunction(rawL, StaticLuaCallbacks.PrintWarning);
                if (0 != LuaAPI.xlua_setglobal(rawL, "print_warning"))
                {
                    throw new Exception("call xlua_setglobal fail!");
                }
                //end
        ...
        public LuaTable CustomGlobal
        {
            get
            {
                if (_CustomG == null)
                {
                    _CustomG = Global.Get<LuaTable>("_G");
                }
                return _CustomG;
            }
        }
        ...
        public void ThrowExceptionFromError(int oldTop)
        {
            ...
            throw new LuaException(LuaPrintFileLink(err.ToString()));
#if THREAD_SAFE || HOTFIX_ENABLE
            }
#endif
        }
        
        internal static string LuaPrintFileLink(string input)
        {
#if UNITY_EDITOR
