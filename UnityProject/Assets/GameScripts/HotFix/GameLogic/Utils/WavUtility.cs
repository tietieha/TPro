...
public class WavUtility
{
    ...
    public static AudioClip ToAudioClip(string filePath)
    {
        ...
        {
            Log.Warning("This only supports files that are stored using Unity's Application data path. \nTo load bundled resources use 'Resources.Load(\"filename\") typeof(AudioClip)' method. \nhttps://docs.unity3d.com/ScriptReference/Resources.Load.html");
            return null;
        }
    ...
    public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
    {
        ...
        //Log.InfoFormat ("riff={0} wave={1} subchunk1={2} format={3} channels={4} sampleRate={5} byteRate={6} blockAlign={7} bitDepth={8} headerOffset={9} subchunk2={10} filesize={11}", riff, wave, subchunk1, formatCode, channels, sampleRate, byteRate, blockAlign, bitDepth, headerOffset, subchunk2, fileBytes.Length);
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
    {
        ...
        if (saveAsFile)
        {
            ...
            //Log.Info("Auto-saved .wav file: " + filepath);
        }
        else
        {
            filepath = null;
    ...
    private static int WriteBytesToMemoryStream(ref MemoryStream stream, byte[] bytes, string tag = "")
    {
        ...
        //Log.InfoFormat ("WAV:{0} wrote {1} bytes.", tag, count);
        return count;
    }
    ...
    private static string FormatCode(UInt16 code)
    {
        switch (code)
        {
            ...
            default:
                Log.Warning("Unknown wav code format:" + code);
                return "";
        }
    }
