...
public class DynamicInfinityListRenderer : MonoBehaviour
{
    ...
    void _SetListRenderSize(int count)
    {
        ...
        {
            mRectTransformContainer.sizeDelta = new Vector2((Mathf.CeilToInt((count * 1.0f / ColumnCount))) * GetBlockSizeX(), mRectTransformContainer.sizeDelta.y);
            mRectMask = new Rect(0, -mMaskSize.x, mMaskSize.x, mMaskSize.y);
        }
    }
    ...
    void _UpdateChildTransformPos(GameObject child, int index)
    {
        ...
        {
            v2Pos.x = CellSize.x /2 + row * GetBlockSizeX();
            v2Pos.y = -column * GetBlockSizeY();
        }
    ...
    void _UpdateDynmicRects(int count)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            DynamicRect dRect = null;
            if (IsVertical)
            {
                dRect = new DynamicRect(column * GetBlockSizeX(), -row * GetBlockSizeY() - CellSize.y, CellSize.x, CellSize.y, i);
            }
            else
            {
                dRect = new DynamicRect(row * GetBlockSizeX() + CellSize.x, -column * GetBlockSizeY(), CellSize.x, CellSize.y, i);
            }
            mDict_dRect[i] = dRect;
        }
    }
    ...
    public virtual void LocateRenderItemAtIndex(int index, float delay)
    {
        ...
        {
            v2Pos = new Vector2(-row * GetBlockSizeX(), pos.y);
        }
