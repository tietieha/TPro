...
public class PlanarReflections : MonoBehaviour
{
	...
	public class PlanarReflectionSettings
	{
		...
		public Camera m_WillReflectCamera;
	...
	private int planarReflectionTextureID = Shader.PropertyToID("_ReflectionTex");

	private Vector2Int m_OldReflectionTextureSize;

	private void OnEnable()
	{
		float useReflect = GetComponent<Renderer>().sharedMaterial.GetFloat("UseReflact");
		if (useReflect < 0.5f) ////不开反射就不运行，本来是1，0
		{
			enabled = false;
			return;
		}
	...
	private Camera CreateMirrorObjects(Camera currentCamera)
	{
		...
		UnityEngine.Rendering.Universal.UniversalAdditionalCameraData lwrpCamData =
			go.AddComponent(typeof(UnityEngine.Rendering.Universal.UniversalAdditionalCameraData)) as
				UnityEngine.Rendering.Universal.UniversalAdditionalCameraData;
		UnityEngine.Rendering.Universal.UniversalAdditionalCameraData lwrpCamDataCurrent =
			currentCamera.GetComponent<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
	...
	public void ExecuteBeforeCameraRender(ScriptableRenderContext context, Camera camera)
	{
		if (!enabled)
			return;
		if (camera.cameraType == CameraType.Game && m_settings.m_WillReflectCamera != null && camera != m_settings.m_WillReflectCamera)
			return;
		...
		var res = ReflectionResolution(camera,
			UnityEngine.Rendering.Universal.UniversalRenderPipeline.asset.renderScale);
