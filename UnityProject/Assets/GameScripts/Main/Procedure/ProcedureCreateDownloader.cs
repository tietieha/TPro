...
namespace GameMain
{
    public class ProcedureCreateDownloader : ProcedureBase
    {
        ...
        private async UniTaskVoid CreateDownloader()
        {
            ...
            {
                GameEvent.Send(ProcedureEventDefines.Events.PROCEDURE_RES_CHECK_VERSION_SUCCESS, "0");
                TEngine.Log.Info("Not found any download files !");
                ChangeState<ProcedureDownloadOver>(_procedureOwner);
            }
            else
            {
                //A total of 10 files were found that need to be downloaded
                TEngine.Log.Info($"Found total {_downloader.TotalDownloadCount} files that need download ！");
                ...
                GameEvent.Send(ProcedureEventDefines.Events.PROCEDURE_RES_CHECK_VERSION_SUCCESS, _totalSizeMb);

                float availableSpace = SDKHelper.GetAvailableStorage();
                Log.Info($"Available space: {availableSpace}MB, Required size: {sizeMb}MB");
                if (availableSpace > 0.1f && availableSpace < sizeMb)
                {
                    GameEvent.Send(ProcedureEventDefines.Events.COMMON, ProcedureEventDefines.Command.PROCEDURE_RES_PATCH_SHOW);
                    UILoadTip.ShowMessageBox(LoadText.Instance.Label_Storage_Low_Load,
                        MessageShowType.OneButton,
                        LoadStyle.StyleEnum.Style_QuitApp, Application.Quit);
                    return;
                }

                float maxSizeMb = GameModule.Resource.MaxCarrierDataNetworkSizeMb;
                if (sizeMb >= maxSizeMb && Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork)
                {
                    GameEvent.Send(ProcedureEventDefines.Events.COMMON, ProcedureEventDefines.Command.PROCEDURE_RES_PATCH_SHOW);
                    UILoadTip.ShowMessageBox(LoadText.Instance.Label_Net_ReachableViaCarrierDataNetwork,
                        ...
                        () =>
                        {
                            GameEvent.Send(ProcedureEventDefines.Events.COMMON, ProcedureEventDefines.Command.PROCEDURE_RES_PATCH_CONFIRM);
                            StartDownFile();
        ...
        private async UniTaskVoid RequestUpdateData()
        {
            ...
            var checkVersionUrl = SettingsUtils.GetResCheckVersionURL();
            ...
            TEngine.Log.Info("RequestUpdateData, proxy:" + checkVersionUrl);
