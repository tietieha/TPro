...
#pragma warning restore CS0414
        ...
        protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
        {
            ...
            UILoadMgr.Show(UIDefine.UILoadUpdate, $"载入中...");
            LoadUpdateLogic.Instance.DownProgressAction?.Invoke(80f);
            LoadAssembly().Forget();
        }

        private async UniTaskVoid LoadAssembly()
        {
            GameEvent.Send(ProcedureEventDefines.Events.COMMON, ProcedureEventDefines.Command.PROCEDURE_SCRIPT_START);
        ...
        {
            GameEvent.Send(ProcedureEventDefines.Events.COMMON, ProcedureEventDefines.Command.PROCEDURE_SCRIPT_SUCCESS);
            ...
            {
                TEngine.Log.Fatal($"Main logic assembly missing.");
                return;
            }
            GameModule.GameAppAssembly = m_MainLogicAssembly;

            var appType = m_MainLogicAssembly.GetType("GameApp");
            if (appType == null)
            {
                TEngine.Log.Fatal($"Main logic type 'GameMain' missing.");
                return;
            }
            GameModule.GameAppType = appType;

            var entryMethod = appType.GetMethod("Entrance");
            if (entryMethod == null)
            {
                TEngine.Log.Fatal($"Main logic entry method 'Entrance' missing.");
                return;
            }
