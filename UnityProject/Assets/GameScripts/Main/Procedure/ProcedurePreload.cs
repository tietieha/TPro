...
namespace GameMain
{
    ...
    public class ProcedurePreload : ProcedureBase
    {
        ...
        private void OnPreLoadAssetFailure(string assetName, LoadResourceStatus status, string errormessage, int guid, object userdata)
        {
            TEngine.Log.Warning("Can not preload asset from '{0}' with error message '{1}'.", assetName, errormessage);
            _loadedFlag[assetName] = true;
        }

        private void OnPreLoadAssetSuccess(string assetName, object asset, int guid, object userdata)
        {
            TEngine.Log.Debug("Success preload asset from '{0}' duration '{1}'.", assetName, guid);
            _loadedFlag[assetName] = true;
        }
    }
}